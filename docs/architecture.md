...
- `VerifiedIntegrationReadiness`：位于`shared/runtime_test/integration_readiness_gate.py`的preflight-only能力，严格重读上述 PASS receipt 后得到进程内、本地兼容检查结果。它复核 manifest、endpoint、access policy、catalog、每个 dataset 的 schema/query/evidence policy、上海交易日和语义快照，并拒绝宽松权限、symlink/hardlink、重复 JSON key、跨日或配置漂移；进程 HMAC 只能防止普通对象在同一进程冒充检查结果。receipt 的自包含 SHA 不是签名，这个对象不认证来源、不证明 probe 真实运行，也不解锁 runtime、capital 或 Journal writer。`shared/runtime|portfolio|capital|risk|execution`不得导入该模块，该模块也不得依赖这些交易authority或SampleJournal writer；实际 dataset 是否可用仍只能由当次 `DataEvidenceGate` 决定。
...
IntegrationReadinessProbe（显式 fixture/移交 manifest；非 authority）
  ...
  -> 严格重读为进程内 VerifiedIntegrationReadiness
  -> exact runtime config 绑定后才允许尝试下方本地链
SharedSignalsV1Client（fixture transport）
  ...
  -> frozen SampleJournal outcome/counterfactual/calibration projections（仓外只读）
...
`RunContext` 把 `decision_as_of` 与 `trade_date` 按 `Asia/Shanghai` 绑定，并将它纳入 run identity。两套本地能力不得混称为一条已上线链：`compose_paper_runtime`/fixture CLI 仅接受精确的数据化 `FrozenFixtureStagePort` 和 fixture 账户/proof，可执行但非authority；`compose_capital_backed_paper_runtime` 使用 public capital/risk/execution/reconcile ports，从 canonical simulated ledger、人工选择Champion、六维论点风险authority与持久drift authority组合，但仍只有 test-only 装配，没有 CLI 或 scheduler。独立审计发现离线 no-action 路径仍会追加唯一模拟资本和 Journal 事实，因此该入口已删除，而不是以“零订单”包装成无副作用。当前没有 live SS、真实provider或真实paper样本。所有入口都不接受任意 callable 通过自报旗标冒充“离线”。Risk和网络关闭simulation的每笔资本/执行副作用都必须重读最新drift与Champion authority；论点风险链另行复核policy/proof/exposure-set identity、每笔精确notional delta、同股票组连续性和最终exposure map。收紧时把剩余open/increase强制改为未成交，同时保留权威reduce/exit和必要reservation清理。无新增订单但存在明确阻断时，paper day结束为`completed_with_blocks`。这不证明未来live broker已关闭TOCTOU；真实外部副作用前必须另做同等authority检查。RunBundle store只使用显式本地root，以逐事件不可变文件、fsync和原子publish支持恢复；业务receipt不封存主机绝对Journal路径，reported stage只封存相对publisher root的稳定定位，使同输入跨输出根保持相同bundle内容地址。fixture CLI只写隔离artifact、不写正式Journal，也不能自动晋级。
...
离线科学闭环使用 `shared/runtime_test/ashare_offline_science.py` 对显式 `as_of` 的 frozen SampleJournal view 与预测前冻结的ValidationPlan生成内容寻址、仓外projection。`FrozenJournalView`在构建与验收前从完整source events重建cutoff分区、excluded/max evidence与included head，并用进程内HMAC seal绑定原始source digest/byte count及内部索引；整包拒绝subclass，只能从这一个精确类型的已验视图派生events与metadata。该seal不是外部签名、外部密封或生产durable authority；public exact verifier另强制调用方独立传入预期`as_of`与`authority_scope`，报告内容不得自己定义验收锚点。Outcome从精确source events、plan和artifact provenance重建：内联标签永不进入统计，只接受同authority/同identity的forward-label update；trade date由上海时间与冻结calendar推导；A股target/evidence必须精确同刻；reference/exit payload、hash、PIT、price、cost、direction return与label结果逐项重算。plan provenance和market truth均通过无默认verifier端口，调用方或artifact自报可信不能提升；没有这两个外部authority时仍可保留observation-only结果，但不能进入统计。下游四份报告及五文件bundle必须携带精确source与同一verifier重建，不能接受仅重算自哈希的报告。counterfactual只投影费用后回报，不创建虚构资本或PnL账本；metrics与calibration共同只使用eligible且每cluster一条的无歧义cohort，并分别显示eligible cluster数、观察交易日数、propensity权重Kish有效样本量与依赖修正样本量，按最长主horizon用1/3/5日移动观察交易日块bootstrap，少于两个完整块时不发布区间；这不证明序列真正独立。MG ablation只接受同一PIT、成本和配对身份；small-account sensitivity固定50k/15%/90%/100股硬策略并禁止从扫描结果选择winner。当前手工CLI没有生产plan-provenance或market-truth verifier，因此不会产生eligible科学样本。任何投影都不能回写SampleJournal、替代冻结OOS、自动晋级或扩风险。
...
`evaluate_ashare_frozen_candidates` 只用于离线评测被捕获的候选输出：黄金标签与候选fixture物理分离，固定dev/OOS membership、每角色尝试预算和`oos_used_for_tuning=false`，并检查引用、关键事实、矛盾、时点、注入、敏感信息及交易越权。报告必须保留`candidate_capture_mode=offline_fixture`和`provider_call_verified=false`；因此评测通过只能证明评估器能识别固定样本，不能证明真实DeepSeek质量，更不能把同一LLM生成的标签当作独立裁判。
...
- 已实现可散列`ValidationPlan`、无默认calendar verifier/detached proof、外部计划artifact门、SampleJournal/ops贯穿、同一authority派生的A股forward targets，以及离线Outcome的无默认plan-provenance/market-truth门；但当前只有测试fixture verifier，手工CLI有意不接生产verifier且尚无受信artifact registry，所以真实样本只能保持observation-only。生产calendar/真实market-truth、总回报/公司行动authority、purged/nested walk-forward、PBO、Deflated Sharpe、多重试验和OOS重用审计artifact均未完成；
