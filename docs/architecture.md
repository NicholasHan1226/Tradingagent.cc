...
- TradingDatas 是基础行情、事件、行业、交易日历与合约输入 authority。TradingAgent 只消费 `GET /v1/catalog` 与 `POST /v1/query`，不直读兄弟仓 SQLite，也不现场调用数据提供商；TradingDatas fresh handoff 前只运行 fixture/mock-first。
