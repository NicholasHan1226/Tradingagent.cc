...
- `tradingagent.llm_evidence.v1`：LLM/DeepSeek 只生成固定Prompt、内容寻址且可重新验证的证据观察。source span 被包在明确的不可信数据边界，显式中英文提示注入模式会在transport前阻断；artifact hash只证明内容完整性，外部来源authority仍必须由receipt和独立verifier复核。typed provider receipt分别绑定本地完整transport material、实际供应商outbound、response与标准化evidence hash；内部proof metadata不发送给供应商。成功离线证据可进入显式head-CAS的本地hash-chain journal，`.head`仅为本地完整性锚点，不是外部密封、tamper-proof authority或production durable sink。当前没有真实provider transport。它不能直接改变候选成员、预测分数、风险请求、目标仓位或订单。
...
密钥变量名固定为`DEEPSEEK_API_KEY`，配置对象绝不读取其值，换成其它格式合法的环境变量名同样会被拒绝。任意模型映射只能通过显式`from_offline_fixture_mapping`构造，并带`fixture_only=true`，不能授权provider egress；`LLMRouter`原有的独立环境变量入口已经移除。未来真实transport必须只接受provider专用的validated router，不能复用fixture路由。
...
`shared/llm/deepseek_config.py`把base URL `https://api.deepseek.com`、`deepseek-v4-flash`与`deepseek-v4-pro`映射到`bulk_extraction`和`slow_research`逻辑角色；2026-07-16已从[DeepSeek官方快速入门](https://api-docs.deepseek.com/)和[模型说明](https://api-docs.deepseek.com/quick_start/pricing/)核对这些公开接口目标、JSON输出与thinking能力。配置只保存密钥环境变量名称，拒绝旧别名，并强制`network_enabled=false`。默认Gateway也从这份严格、无密钥配置构造，而不是从宽松模型字符串直接生成路由。官方文档核对不等于认证账户的`/models` readback或真实canary；quota、限流、重试、幂等、成本、数据留存和当前账户可用性均未验证。DeepSeek adapter只接受不可调用的冻结离线响应fixture，request与最终outbound hash任一不匹配即fail closed；真实HTTP transport必须是另一个显式类型，并另行验收URL allowlist、禁重定向、DLP、durable receipt sink和frozen canary。离线候选对`bulk_extraction`构造thinking disabled与`max_tokens=4096`目标，对`slow_research`构造thinking enabled、`reasoning_effort=high`与`max_tokens=8192`目标；本地authority proof metadata通过独立`transport_material_sha256`进入typed receipt，不进入provider outbound。source proof在查找fixture前验证，receipt的`received_at`在响应返回后采样；receipt还绑定标准化evidence hash，journal run ID由该receipt派生。动态Prompt、未验证/未绑定source span、缺外部authority verifier、cutoff后的receipt、未知动态对象，以及provider输出中的credential-shaped字段/值均fail closed；最终transport前对完整键值树执行账户、持仓、现金、broker、order、交易语义、secret DLP，并对source span中的显式角色覆盖/忽略指令做中英文负例阻断。命中后不查询fixture，保留引用并标记human review。该检测器不是所有语义型或编码型注入的完备证明；本地journal可发现断链、swap、partial tail和anchor不一致，但同时替换或删除journal与本地anchor仍不等于外部可证明防篡改。真实provider调用、production durable receipt authority、冻结评测、延迟/成本和收益增量仍未验证。
...
- DeepSeek provider transport 和 live paper scheduler 仍为 `PLANNED_NOT_IMPLEMENTED`；
