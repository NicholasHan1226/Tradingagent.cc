...
- 已建立用户申报资金/持仓、关注股、个股正反证据、条件门、组合人工复核、正式个股投影 publisher/receipt 和人工计划/复盘账本。下一运行门是用当前 TradingDatas 回执持续发布所关注/持仓股票；代码可打包的数量不等于当前数据覆盖。
- 多周期正式图线仍取决于 TradingDatas 已验收的历史日线/分钟窗口；没有历史PIT、公司行动/复权和连续回执时，对应周期保持空值。Kronos 仅在冻结OOS、独立校准、覆盖率、成本和同样本基线全部通过后进入影子比较，不能靠安装权重关闭该门。
- 公告/新闻已具备正式事件回执入口；真正的聚合舆情方向仍需要独立数据集与分类回执。没有该输入时不得把新闻摘要或讨论热度自动转成情绪概率。
