...
> 阅读顺序：[../AGENTS.md](../AGENTS.md) → [../STATUS.md](../STATUS.md) 与本轮新鲜运行读回 → 本文件。规划入口是 [EVOLUTION_PROGRAM.md](EVOLUTION_PROGRAM.md)。Controller 的跨线编排/事后审计及 Mac mini 控制面归属见根 `AGENTS.md`；其缺席不阻断已绿候选合并，也不替代直接运行证据。
