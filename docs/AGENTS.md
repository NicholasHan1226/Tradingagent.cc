...
> 阅读顺序：[../AGENTS.md](../AGENTS.md) → `../../autodev-control/AUTODEV_STATE.json` 与本轮新鲜运行读回 → [../STATUS.md](../STATUS.md) 历史快照 → 本文件。
...
根 `README.md` 只做入口；根/模块 `AGENTS.md` 保存长期规则；AutoDev 状态保存跨线当前机器事实，运行结论由本轮读回确认；`STATUS.md` 是 2026-08-02/03 历史快照。不要重新创建重复的 infrastructure、data-source、write-end、事故日记或实施计划文档。

## 更新规则

- 资本/架构/安全边界变化：更新 `architecture.md` 和最近层 `AGENTS.md`。
- schema、路径、字段或事实源变化：更新 `data_contract.md`。
- 能力状态、验证层或退役阶段变化：更新 `system_state_matrix.md` 和对应机器治理记录；新鲜运行事实不得回写历史 `STATUS.md`，而应由状态入口和同轮 readback 分层记录。未完成复核时采用更严格状态。
...
- 当前测试、远端、生产、cron 或真实市场证据：写入机器状态/运行报告并按层级披露，不覆盖历史 `STATUS.md`。
- 后续范围：只写入 `BACKLOG.md`；不得把 backlog 写成已实现或已授权。

`superpowers/plans/` 是实现任务记录，不是 current 能力证明。计划中的勾选、进度说明或候选文件存在，都必须回到机器状态、测试和本轮运行证据分层核验；历史 `STATUS.md` 只提供旧语境。
