...
后续防线已补在 TA 的 TD API 消费适配器：对 API 明确标记为可恢复的 429/503、网络超时
和底层传输错误，按 2/5/10/20 秒做最多四次重试；401/403、字段/分页/目录合同错误以及
新鲜度失败不重试，仍按原有逐分片隔离和 fail-closed 处理。该机制只是跨越短暂锁竞争的
消费侧容错，不把 API 失败转换成成功，也不替代 TD 读模型的长期解耦；下一自然窗口仍须
同时记录 TD 采集器状态、TA 运行结果、fresh receipt 和 consumer readback。
