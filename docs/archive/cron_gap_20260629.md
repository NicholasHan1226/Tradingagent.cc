...
> **历史参考，不作为当前运行依据。** 本文件是 2026-06-29 的一次性快照，记录旧 MarketGraph crontab 向 TradingAgent/SharedSignals 拆分前的迁移差距。当前权威状态以 [../../STATUS.md](../../STATUS.md)、仓库内 `crontab.txt`、`shared/wrappers/` 和服务器 live crontab 为准。
