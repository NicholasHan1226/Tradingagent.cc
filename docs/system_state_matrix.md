...
| A股 observation timer | worker unit 已按root-owned versioned minimal runtime安装为inactive/static；PyYAML版本/wheel hash和UID987真实入口已验，timer仍不存在 | `TARGET_CONTRACT / server_validated_non_authority_read_only / production=false` | 没有current切换、动态manifest rollover或timer激活；不能用静态manifest/as-of冒充日更 |
...
| `tradingagent_ashare_observation_timer_candidate` | `TARGET_CONTRACT / architecture_target` | worker unit已安装为inactive/static、runtime已验；timer不存在，动态manifest rollover未通过 |
