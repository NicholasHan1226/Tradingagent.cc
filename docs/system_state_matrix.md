...
> 本文解释系统状态治理；当前能力状态的机器可读事实源是 `shared/governance/system_state_matrix.yaml`，旧链消费者与退役门的机器可读事实源是 `shared/governance/legacy_inventory.yaml`。本地候选、Git 主线、服务器旁路、生产文件、生产 runtime、已安装 cron、真实数据与真实交易动作必须分别验证。既有 A股 V1 基线已进入 Git 主线；当前 `codex/ta-market-kernel-retirement-v1` 的市场内核、退役和三市场 lane 变化仍是 dirty、未提交的本地隔离候选。`layer=local_isolated_candidate`与`production_verified=false`只证明对应 allowed uses，不能把代码存在静默提升为 main、runtime、真实数据或交易能力。
...
- Mini/Hermes webhook、file consumer 与 `RealSignalQueue` 在本地候选源码层已退役并由 `tradingagent_mini_hermes_retirement` 阻止恢复；A股只保留`tradingagent.ashare.paper_broker.v1`的server-local模拟合同。`ASHARE_SIM_HERMES_ENABLED=0`和`ASHARE_SIM_WEBHOOK_ENABLED=0`仅是安装态清理墓碑，不代表服务器或Mini上的cron、env、process、port已经清零；这些仍需独立只读readback。
...
| TA TradingDatas V1 client + Evidence Gate | 实现与契约测试候选存在；兼容代码符号仍含`SharedSignalsV1*` | `CURRENT_VERIFIED / local_isolated_candidate / production=false` | 仅fixture/contract；不代表TradingDatas runtime |
| TA TradingDatas V1 integration-readiness probe | 显式manifest、统一`as_of`双跑、必需字段/行级PIT/receipt/内容哈希与脱敏回执；`next_cursor`在跨页合同冻结前阻断 | `CURRENT_VERIFIED / local_isolated_candidate / production=false` | 仅fixture或另行授权的只读联调；不是TradingDatas验收owner、每日监控、生产或交易authority |
...
| market evidence + execution clock | mark/quote绑定dataset/catalog/source receipt/lineage/calendar/capital context；显式fixture clock在sim submit与capital commit前分别重验freshness/session | `CURRENT_VERIFIED / local_isolated_candidate / production=false` | 本地hash不是签名；fixture verifier/clock不证明TradingDatas live、交易所行情或生产时间authority |
...
| capital-backed composition | canonical simulated account、current generation/lineage、人工选定Champion、逐副作用authority门、capital outbox、risk/execution/reconcile组合候选存在 | `CURRENT_VERIFIED / local_isolated_candidate / production=false` | 仅test-only装配；无CLI、scheduler、live TradingDatas或真实paper sample |
| LLM provenance router + journals | 显式typed recorder只接受一个绝对accepted锚点及其确定性伴随路径，三类Journal端点构造后只读；逻辑内容键不依赖调用方ID，provider-invocation Journal在网络前落`in_flight`并于同一canonical family内持跨进程锁至唯一终态，accepted/rejected互斥结果分别写入物理隔离结果Journal；六个data/head端点互异，unknown mode、伪recorder、ID冲突、未知in-flight、文件身份或持久化失败均fail closed | `CURRENT_VERIFIED / local_isolated_candidate / production=false` | 仅local-integrity-only研究/审计；未验证跨主机共享锚点，不是provider attestation、外部durability、模型可用性或任何交易authority |
...
| Mini/Hermes 与 RealSignalQueue 退役 | 专用源码、网络发送器和Mini消费者已删除；旧配置truthy fail closed | `RETIRED_BLOCKED / local_isolated_candidate / production=false` | 仅证明候选源码退役；不证明已安装服务器或Mini运行态已清理 |
| 三市场长期 lane 与 BrokerAdapter 边界 | worktree/branch/path owner及互异合同已登记，live统一关闭 | `CURRENT_VERIFIED / local_isolated_candidate / production=false` | 仅本地治理门；不证明任一未来live adapter已实现或部署 |
| A股 market-kernel 变化 | 当前工作树含 server-local paper、T+1及旧执行面退役调整 | dirty local candidate / fresh acceptance pending | 不证明已提交、合入、部署或能连接券商 |
| CNFutures market-kernel 变化 | 当前工作树含独立期货 paper、会话/保证金/证据调整 | dirty local candidate / fresh acceptance pending | 不证明 CTP/SimNow、真实规格或 live adapter 可用 |
| Crypto market-kernel 变化 | 当前工作树含 fixture/mock paper 与独立 Testnet/Live 合同边界调整 | dirty local candidate / fresh acceptance pending | 不证明 7x24 scheduler、Binance Testnet 或 Live 已连接 |
...
|---|---|---|
| `sharedsignals_v1_query` | `TARGET_CONTRACT / local_isolated_candidate` | 保留的机器条目ID；上游 TradingDatas owner 负责，TA 仅等待冻结合同 |
| `tradingagent_sharedsignals_v1_client` | `CURRENT_VERIFIED / local_isolated_candidate` | 保留的兼容机器条目ID；mock-first TradingDatas V1 consumer，不证明 live TradingDatas |
| `tradingagent_sharedsignals_v1_integration_probe` | `CURRENT_VERIFIED / local_isolated_candidate` | 保留的兼容机器条目ID；显式manifest的同`as_of`只读接入门与非authority回执，未运行live TradingDatas，分页合同未冻结时阻断 |
...
| `tradingagent_llm_evidence_journal` | `CURRENT_VERIFIED / local_isolated_candidate` | accepted/rejected物理隔离结果Journal加独立provider-invocation仲裁Journal；本地CAS、hash-chain、`.head`完整性锚点和深层不可变readback；descriptor只作非权威结构校验，不重建typed receipt；不是外部密封、tamper-proof authority或生产durable sink |
| `tradingagent_llm_provenance_router` | `CURRENT_VERIFIED / local_isolated_candidate` | 显式recorder、canonical Journal family、稳定request ID、ID无关逻辑内容键、family内跨进程in-flight仲裁、互斥结果路由与终态精确重放；非canonical/相对/别名路径、unknown mode、未知in-flight或持久化失败fail closed；未验证跨主机共享锚点，无生产或交易authority |
...
| `tradingagent_mini_hermes_retirement` | `RETIRED_BLOCKED / local_isolated_candidate` | 专用源码禁止恢复；零值墓碑等待独立安装态readback，不证明生产已清理 |
| `tradingagent_market_lane_governance` | `CURRENT_VERIFIED / local_isolated_candidate` | A股、CNFutures、Crypto长期lane和互异BrokerAdapter合同；无live实现或部署证明 |
...
| `tradingagent_production_runtime` | `CURRENT_VERIFIED / production_runtime_read_only_snapshot` | 2026-07-19 只读 readback；现役仍为旧 SHA，detached sidecar候选未激活 |
