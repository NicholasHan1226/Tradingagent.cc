...
- TradingDatas `GET /v1/catalog` 和 `POST /v1/query` 由上游唯一 writer/reviewer 负责；TA 只实现可配置 consumer，不读取或修改 TradingDatas 仓，也不把 HTTP 200 当成 dataset 可用。TA 的 Bearer transport 合同只接受受限 token file，并将 header 绑定到 canonical authority 的精确 method/path；认证失败无 fallback。2026-07-22 TA 已在隔离服务器候选用现有 `marketgraph` 只读凭据完成一次 formal loopback authenticated observation 与幂等重放，因此新增状态仅为 `server_validated_non_authority_simulation_only`。该 `a7488e9` 旧 one-shot 没有 membership ledger/history/planner，不得继承为新五项 committed binding 的运行证据。2026-07-23 本地候选新增 catalog-driven 九数据集 parity 合同和专用 `tradingagent` sysusers/front unit 合同；它们当前均是 `TARGET_CONTRACT / repository_contract / production=false`，不证明服务器 UID/GID、TA-scoped token、service 切流或 timer 已完成。MarketGraph 只以 front 读取旧投影所需的临时 supplementary group 存在，不是 TA 服务身份或关键路径。旧 SharedSignals runtime/route/dual-registry 不是新架构依赖。
...
|---|---|---|---|
| TradingDatas V1 upstream query | TA不写TradingDatas；隔离候选已做一次 formal provider-native readback，专用 TA 身份的持续 handoff 仍未完成 | `TARGET_CONTRACT` | 不代表 TA 生产调度、历史 PIT 或交易通过 |
...
| TA TradingDatas catalog parity | 外部无密钥manifest必须与`/v1/catalog`发现的active集合、schema、fields、limits和190/9/181计数精确一致；每个active数据集双跑有界query，ready可接受、impaired只作零权重记账 | `TARGET_CONTRACT / repository_contract / production=false` | parity receipt永不直接晋级research snapshot；当前只证明本地候选与fixture，不证明TA token或服务器readback |
| TA dedicated service identity candidate | tracked sysusers与front unit目标为`tradingagent:tradingagent`；front保持只读/sim-only且显式不可读取`/run/secrets/tradingagent`，迁移期仅通过supplementary group读取旧投影 | `TARGET_CONTRACT / repository_contract / production=false` | 不证明服务器UID/GID、token leaf、unit安装、front切流或timer已激活 |
...
| A股 observation timer | unit/timer 字节仅作 disabled 静态候选；专用 `tradingagent:tradingagent` token/identity 和 daily immutable manifest rollover 尚未 handoff | `TARGET_CONTRACT / architecture_target / production=false` | 启用前 P1；当前 non-enableable，不得用静态manifest/as-of冒充日更 |
...
| `tradingagent_tradingdatas_catalog_parity` | `TARGET_CONTRACT / repository_contract` | catalog驱动的九数据集只读parity候选；5 ready可接受、4 impaired零权重记账，永不直接晋级研究快照；尚无专用TA身份服务器readback |
| `tradingagent_dedicated_service_identity_candidate` | `TARGET_CONTRACT / repository_contract` | tracked `tradingagent` sysusers/front unit候选；尚未创建服务器UID/GID、安装TA token或切换现役service |
...
| `tradingagent_ashare_observation_timer_candidate` | `TARGET_CONTRACT / architecture_target` | 当前 non-enableable；缺专用 identity/token 与 daily immutable manifest rollover |
