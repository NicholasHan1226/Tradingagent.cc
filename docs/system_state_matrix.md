...
> 本文解释系统状态治理；当前能力状态的机器可读事实源是 `shared/governance/system_state_matrix.yaml`，旧链消费者与退役门的机器可读事实源是 `shared/governance/legacy_inventory.yaml`。本地候选、Git 主线、不可变服务器 release、现役 runtime、真实数据与真实交易动作必须分别验证。截至 2026-07-28，专用 TradingAgent 身份、A股核心三数据集 current-session、正式 `rt_min` 30只主板真实30/30 observation readback 属于 `server_validated_non_authority_read_only`。TA仓库支持500只首批扫描与最多6000只主板容量，但500只生产覆盖仍未验证。TradingDatas owner 已启用自身5分钟采集timer；TA simulation-only分钟累计与次日会话timer已启用。首轮分钟自动触发因当日缺口失败关闭且账本不变，尚无成功自动模拟轮次或真实交易，front保持退役停机态。
...
- TradingDatas `GET /v1/catalog` 和 `POST /v1/query` 由上游唯一 writer/reviewer 负责；TA 只实现可配置 consumer，不读取或修改 TradingDatas 仓。2026-07-27 上游 fresh handoff 报告正式目录为 `catalog_version=v1-ee2dbdf4ecc91390`、190/92/98，其中新 63 项为 49 success nonempty、14 legitimate empty、0 execution error，但都如实保持 partial/degraded；`forecast` 与 `pledge_detail` 因必需 `ts_code` 已 paused/404。该 handoff 是上游事实，不替代 TA 自身读回。TA 历史上已以专用身份对旧 26-active 做两次 `limit=1` 首屏查询：当时 3 ready、9 stale、14 unobserved，门禁 3 accept / 23 reject，查询合同失败为 0。动态 manifest builder 只冻结全部 active inventory，并只预检经审核的 calendar/security-master/daily 三项；catalog active、HTTP 200 或 partial receipt 都不会自动晋级研究。
- `tradingagent:tradingagent` UID/GID 987 与专用 token-file 已完成服务器 handoff。parent 为 `root:tradingagent 0710`，leaf 为 `tradingagent:tradingagent 0600` regular、`nlink=1`；Linux 通过 `O_PATH` 安全遍历 execute-only parent，不扩大目录读取权限。root-owned versioned Python runtime 已以 UID 987 通过真实入口和依赖 audit；worker unit 已安装为 inactive/static，timer 仍不存在。front 为 inactive/disabled/runtime-masked，tracked base unit 与仓库字节一致，active drop-in 和旧 8082 引用已清零，8787 closed。不可变 release 均未切 current。
...
- `shared/crontab.txt` 是仓库调度模板，不是已安装 cron。服务器 readback 有恰好一个 pause marker，未发现引用 TradingAgent 仓库或服务的 active job；另有独立旧 A股 closing-scan 由其它 owner 管理，不能当成 TA scheduler。front 与 worker 均未启动，observation timer 不存在，`production_verified=false` 保持不变。
...
| A股5分钟 fixture research loop (`tradingagent_ashare_minute_fixture_research_loop`) | strict/delayed双层Evidence Gate、按真实decision time选择首根可达K线、原子one-shot runner、未校准rank、四个反事实账本与500→6000监控容量已进入仓库 | `CURRENT_VERIFIED / repository_contract / production=false` | 500/6000是代码容量而非生产覆盖；无校准概率、预测收益、durable capital/outbox、scheduler、broker或生产样本 |
| A股正式分钟 observation handoff (`tradingagent_ashare_minute_observation_handoff`) | TradingDatas 已从10只扩到30只并完成正式18082真实30/30回读；TD timer active；50/100/200只批次上游压测完整 | `CURRENT_VERIFIED / server_validated_non_authority_read_only / production=false` | 500只动态分片尚未正式回读；约晚一根5分钟K线，无L1/bid-ask、TA timer、模拟成交或资金authority |
| A股真实分钟 delayed-paper one-shot (`tradingagent_ashare_minute_delayed_paper_one_shot`) | 专用UID以正式18082的30只精确bar手工推进原子runner；八个快照均30/30、无Evidence Gate拒绝，四个50k fixture账本对账，未成交、首根可达bar模拟成交、T+1持仓与pending订单进入Decision Ledger；重复快照失败关闭且状态不变 | `CURRENT_VERIFIED / server_validated_non_authority_simulation_only / production=false` | 仅non-production fixture；TA timer、durable capital、校准概率、broker和真实交易均未启用 |
| A股分钟自动累计候选 (`tradingagent_ashare_minute_auto_accumulator`) | `437fa27…` current、secret-free env与tracked service/timer已安装启用；14:25:44自动触发因真实缺口退出2，timer继续active且正式bundle不变 | `CURRENT_VERIFIED / server_enabled_fail_closed_simulation_scheduler / production=false` | 尚无成功自动模拟轮次、新交易日09:35首轮、跨日恢复或连续运行证明 |
| A股分钟会话初始化器 (`tradingagent_ashare_minute_session_initializer`) | 动态catalog/calendar/daily初始化器与09:20 timer已安装启用；16:30日线后已用专用UID在隔离root完成次日30/30输入、Universe SHA与幂等重放，正式次日目录保持不存在 | `CURRENT_VERIFIED / server_enabled_preopen_pending / production=false` | 隔离PASS不替代09:20正式timer读回或首个成功自动轮；不以identityless停牌数据生成执行证据 |
...
| `tradingagent_ashare_observation_manifest_rollover` | `CURRENT_VERIFIED / repository_contract` | 99-active服务器读回证明daily stale时正确fail-closed；尚无新manifest、worker或timer激活 |
...
| `tradingagent_ashare_minute_fixture_research_loop` | `CURRENT_VERIFIED / repository_contract` | strict/delayed分钟证据、首根可达bar结算、原子runner、500→6000监控容量和四反事实账本；无durable authority或runtime |
| `tradingagent_ashare_minute_observation_handoff` | `CURRENT_VERIFIED / server_validated_non_authority_read_only` | 正式18082已真实读回30只主板5分钟K线；约晚一根bar，仅 observation/data accumulation，无500只生产证明、TA timer或成交authority |
| `tradingagent_ashare_minute_delayed_paper_one_shot` | `CURRENT_VERIFIED / server_validated_non_authority_simulation_only` | 正式30只分钟证据已驱动手工原子runner、首次可达bar模拟成交、Decision Ledger、四个隔离fixture账本和重复快照失败关闭；无TA timer、durable capital、broker或真实交易 |
| `tradingagent_ashare_minute_auto_accumulator` | `CURRENT_VERIFIED / server_enabled_fail_closed_simulation_scheduler` | timer enabled/active；首轮自动触发因当日缺口失败关闭且bundle不变，不宣称已有成功自动模拟轮次 |
| `tradingagent_ashare_minute_session_initializer` | `CURRENT_VERIFIED / server_enabled_preopen_pending` | 09:20 timer enabled/active；正式18082的次日30/30输入已在隔离root PASS并幂等重放，正式09:20目录与首个自动轮仍待读回 |
...
| `tradingcopilot_v1` | `CURRENT_VERIFIED / repository_contract` | A股人工决策页与独立用户申报状态；支持资金/持仓/关注、组合人工复核、分离的人工计划与复盘；正式批量投影绑定行情/主数据/事件 detached receipts；基线/Kronos只做冻结OOS非晋级评估；不连接券商、不写量化资本/订单/样本/晋级；运行批次覆盖仍须独立验证 |
