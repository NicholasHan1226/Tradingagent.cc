...
| A股分钟会话初始化器 (`tradingagent_ashare_minute_session_initializer`) | 动态catalog/calendar/daily初始化器与09:20 timer已安装启用；目标日历缺失时失败关闭且未创建次日目录 | `CURRENT_VERIFIED / server_enabled_preopen_pending / production=false` | 尚未完成真实次日pre-open PASS；不以identityless停牌数据生成执行证据 |
...
| `tradingagent_ashare_minute_session_initializer` | `CURRENT_VERIFIED / server_enabled_preopen_pending` | 09:20 timer enabled/active；动态catalog/calendar/daily只读准备次日输入，真实pre-open PASS尚未完成 |
