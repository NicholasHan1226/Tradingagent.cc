...
| `tradingagent_llm_evidence` | `CURRENT_VERIFIED / local_isolated_candidate` | evidence-only sidecar；有typed injected transport receipt合同，无真实DeepSeek网络或durable sink |
...
| `tradingagent_deepseek_provider_config` | `CURRENT_VERIFIED / local_isolated_candidate` | 官方公开接口目标已核对；密钥变量名固定为`DEEPSEEK_API_KEY`且不读取值，任意模型映射仅为`fixture_only`；只允许request+outbound绑定冻结响应，认证账户readback/canary与网络transport仍不存在 |
...
| `tradingagent_deepseek_provider_transport` | `PLANNED_NOT_IMPLEMENTED / architecture_target` | DeepSeek 网络 transport 未实现 |
