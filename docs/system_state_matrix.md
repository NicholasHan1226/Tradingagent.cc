...
| A股分钟自动累计候选 (`tradingagent_ashare_minute_auto_accumulator`) | `b7263e8…` current、secret-free env 与 tracked service/timer 已安装；本地3782项及服务器57项测试通过；真实当日缺口和旧行/新receipt隔离回放均失败关闭且正式bundle不变 | `CURRENT_VERIFIED / server_installed_disabled_simulation_only / production=false` | timer disabled/inactive；尚无新交易日09:35首轮、连续自动触发、跨日初始化或崩溃恢复证明 |
| A股分钟会话初始化器 (`tradingagent_ashare_minute_session_initializer`) | 动态catalog、目标交易日、pretrade_date与审核Universe上一交易日close绑定后，原子准备三项私有输入；不创建state bundle | `CURRENT_VERIFIED / repository_contract / production=false` | 尚未安装或运行真实次日pre-open；不以identityless停牌数据生成执行证据 |
...
| `tradingagent_ashare_minute_auto_accumulator` | `CURRENT_VERIFIED / server_installed_disabled_simulation_only` | current/unit/env已安装，timer因当日真实缺口保持disabled/inactive；下一交易日从09:35连续启动前不宣称自动模拟 |
| `tradingagent_ashare_minute_session_initializer` | `CURRENT_VERIFIED / repository_contract` | 动态catalog/calendar/daily只读准备次日三项私有输入；不创建账本、订单或执行authority，真实pre-open尚未验 |
