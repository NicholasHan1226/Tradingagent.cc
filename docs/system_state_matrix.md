...
| readiness receipt本地兼容preflight | `shared/runtime_test/integration_readiness_*`重读私有回执并绑定exact runtime config；交易authority双向import隔离 | `CURRENT_VERIFIED / local_isolated_candidate / production=false` | 只证明本地内容完整性与配置兼容；不认证来源、不证明probe执行、不授权writer |
...
| offline science projection | 精确类型的同一frozen SampleJournal view，完整source分区重建 + 进程内seal + 独立expected cutoff/scope + ValidationPlan/provenance精确重建outcome/counterfactual/cost/calibration/MG ablation仓外投影；无默认plan-provenance/market-truth verifier；统计与calibration只消费无歧义eligible cohort；整包exact-source复核；观察交易日/Kish N_eff/重叠horizon依赖修正；固定50k敏感性网格 | `CURRENT_VERIFIED / local_isolated_candidate / production=false` | 进程内seal不是外部签名或durable authority；当前CLI无生产verifier，实际只产observation-only投影；统计近似不证明独立或有效策略；不写Journal/资本/订单，不选winner，不替代冻结OOS或自动晋级 |
...
| A股 LLM frozen evaluation | 分离的gold/candidate内容寻址fixture，冻结dev/OOS、尝试预算和安全/引用/时点维度 | `CURRENT_VERIFIED / local_isolated_candidate / production=false` | `provider_call_verified=false`；不证明真实模型质量、增量或收益 |
...
| `tradingagent_integration_readiness_local_gate` | `CURRENT_VERIFIED / local_isolated_candidate` | runtime_test preflight；只验本地内容完整性与exact config兼容，不能认证来源或授权capital/Journal writer |
...
| `tradingagent_llm_ashare_frozen_evaluation` | `CURRENT_VERIFIED / local_isolated_candidate` | gold/candidate分离、冻结dev/OOS与尝试预算；`provider_call_verified=false`，无交易/晋级authority |
...
| `tradingagent_offline_science_projection` | `CURRENT_VERIFIED / local_isolated_candidate` | 同一已验FrozenJournalView + 独立expected cutoff/scope + ValidationPlan/provenance精确source重建；无默认plan/market-truth authority、共同无歧义cohort、整包复核、移动观察交易日块依赖近似与固定50k工程敏感性；当前CLI只产observation-only，不回写authority、不选winner、不自动晋级 |
