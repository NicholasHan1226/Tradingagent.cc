      ...
      {rows.length ? <div className="terminal-table-scroll">
        <table aria-label="风险事件账本" className="terminal-table">
          ...
          <tbody>{rows.map((row) => <tr key={`${row.symbol}-${row.updatedAt}`}>
            <td><strong>{row.symbol}</strong></td><td>{row.market}</td><td>{row.stage}</td><td><span className="terminal-state negative">{row.gate}</span></td><td>{row.evidence}</td><td className="reason-cell">{row.reason}</td><td>{row.updatedAt}</td>
          </tr>)}</tbody>
        </table>
