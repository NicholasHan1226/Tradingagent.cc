...
export function ProcessCycleLedger({ rows, onSelect, selectedId }: { rows: ProcessCycleRow[]; onSelect?: (id: string) => void; selectedId?: string | null }) {
  return (
    <section aria-label="机会周期账本" className="terminal-table-panel process-cycle-ledger">
      ...
      {rows.length ? <div className="process-cycle-list">{rows.map((row) => <CycleRow key={row.id} onSelect={onSelect} row={row} selected={selectedId === row.id} />)}</div> : <TerminalEmpty title="运行空闲" detail="新机会形成后会按发现、研判、风控、待确认和结果聚合。" />}
    </section>
  )
}

function CycleRow({ row, onSelect, selected }: { row: ProcessCycleRow; onSelect?: (id: string) => void; selected: boolean }) {
  return (
    <button aria-label={`${row.symbol} ${row.result} ${row.id}`} aria-pressed={selected} className={`process-cycle-row ${selected ? 'selected' : ''}`} onClick={() => onSelect?.(row.id)} type="button">
