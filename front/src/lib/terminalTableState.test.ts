...
const rows = [
  ...
  { symbol: 'BTC-USDT', reason: '风险等待', score: 58 },
]

describe('terminal table state', () => {
  it('filters across searchable evidence and sorts selected values', () => {
    expect(filterAndSortRows(rows, 'btc', (row) => `${row.symbol} ${row.reason}`, (row) => row.score, 'asc')[0].symbol).toBe('BTC-USDT')
    expect(filterAndSortRows(rows, '', (row) => row.symbol, (row) => row.score, 'desc').map((row) => row.symbol)).toEqual(['600519.SH', 'BTC-USDT'])
  })
