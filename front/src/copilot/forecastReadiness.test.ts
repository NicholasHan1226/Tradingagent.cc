...
const readyEvidence: ForecastEvidence = {
  ...
  modelManifestBound: true, pointInTimeVerified: true, frozenOosReceiptBound: true,
  calibrationProofAccepted: true, effectiveIndependentSamples: MIN_CALIBRATION_EFFECTIVE_SAMPLES,
  intervalCoverageVerified: true, costPolicyBound: true,
}
...
describe('forecast readiness gate', () => {
  ...
  it('keeps demo forecasts illustrative and hides probability semantics', () => {
    const result = assessForecastReadiness({
      ...
      effectiveIndependentSamples: 0, intervalCoverageVerified: false,
    })
