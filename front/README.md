...
- TradingCopilot 个股详情已实现多周期价格/成交量、默认关闭的未校准预测包络、方向研究情景、公告/新闻/舆论关联和七页签阅读结构；个股首屏使用“行情主区 + 公司资料/证据强度/四层就绪度/舆论温度右栏”。证据强度只接受正式定型合同，普通 TradingAgent `confidence` 只作为未评分观察，不再映射成 0–100 分。正式个股投影必须附带独立 SHA-256 回执、来源与时效、A 股 T+1/一手/涨跌停/ST/停牌/复权口径，以及公告新闻舆论的发布/采集/修订/来源置信/新颖度/影响期限/来源回执。决策记录必须填写理由、触发、失效和可选风险上限，并可单独记录实际动作与复盘；仍只属于人工意图。决策台提供独立的 A 股代码/名称输入入口，即使关注和持仓均为空，也可先打开任意合法 A 股标的，再自行决定是否加入关注或申报持仓；单纯浏览不会写入个人状态。关注列表、全部资金与持仓、决策记录分别使用独立工作区。持仓股票自动纳入关注，纯关注股票不计入资产；个股右栏只显示“当前个股持仓”并提供全部持仓入口。普通 `/?product=copilot` 不自动注入演示资产，界面预览不会写入个人状态；`?product=copilot&demo=1` 或显式环境开关提供可编辑但不持久化的完整演示状态。正式模式在缺少逐标的只读投影时显示不可用；普通 TradingAgent signal 不会被扩写为价格序列或事件内容。
- TradingCopilot 的预测展示固定区分历史实线、研究情景中位线与宽/窄研究包络，并显示 `shadow_uncalibrated` 语义。没有 receipt-bound 模型清单、PIT/修订链、冻结样本外、独立校准、有效样本、区间覆盖率、费用/滑点、同输入基线对照和费用后净效用前，不得改称真实概率、置信区间或交易胜率。Kronos 只作为 Challenger，不拥有建议或交易 authority。
