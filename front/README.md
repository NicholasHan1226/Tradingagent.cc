...
- TradingCopilot 个股详情已实现多周期价格/成交量、未校准预测包络、方向研究情景、公告/新闻/舆论关联和七页签阅读结构；个股首屏使用“行情主区 + 公司资料/证据共识/舆论温度右栏”，关注列表以抽屉方式进入，避免压缩行情图。开发模式使用明确的 `demo_fixture`；正式模式在缺少逐标的只读投影时显示不可用。普通 TradingAgent signal 不会被扩写为价格序列或事件内容。
