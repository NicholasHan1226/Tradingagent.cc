...
- 机会管道：从机会进入、初筛、研究、风控到队列结果的动态筛选过程。优先使用真实 `funnelEvents[]`；没有新信号但有持仓时，只做“持仓流”的动态回放，展示当前持仓、收益贡献、风险检查、继续持有和复盘记录，不伪装成完整交易漏斗。
...
- 首页机会管道优先展示真实事件流；没有实时事件时，只显示等待态、已有信号推导或持仓流回放，不用静态样例或占位粒子伪装成真实筛选。持仓流必须用账户事实表达“多少持仓、多少正贡献、多少需观察”，并用细线和低饱和状态色展示损耗与流动。没有真实机会、信号或持仓时，管道必须保持轻量等待态，不展示五段零值漏斗。
