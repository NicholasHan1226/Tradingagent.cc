...
- TradingCopilot 个股详情已实现多周期价格/成交量、未校准预测包络、方向研究情景、公告/新闻/舆论关联和七页签阅读结构；个股首屏使用“行情主区 + 公司资料/证据共识/舆论温度右栏”。决策台提供独立的 A 股代码/名称输入入口，即使关注和持仓均为空，也可先打开任意合法 A 股标的，再自行决定是否加入关注或申报持仓；单纯浏览不会写入个人状态。关注列表、全部资金与持仓、决策记录分别使用独立工作区，不再以抽屉或当前个股编辑器冒充完整账户视图。持仓股票自动纳入关注，纯关注股票不计入资产；个股右栏只显示“当前个股持仓”并提供全部持仓入口。普通 `/?product=copilot` 不自动注入演示资产，界面预览不会写入个人状态；`?product=copilot&demo=1` 或显式环境开关提供可编辑但不持久化的完整演示状态。正式模式在缺少逐标的只读投影时显示不可用；普通 TradingAgent signal 不会被扩写为价格序列或事件内容。
- TradingCopilot 的预测展示固定区分历史实线、研究情景中位线与宽/窄研究包络，并显示 `shadow_uncalibrated` 语义。没有样本外校准、覆盖率、漂移和费用后验证前，不得改称真实概率、置信区间或交易胜率。
