...
- 过程：Process Book 展示流程、阶段、状态、证据、耗时和结果；没有运行中记录时自动展示最近完成结果。
- 持仓：Portfolio Ledger 展示市值、组合权重、浮动盈亏、贡献和风险；A股市值使用人民币，多币种不做虚假汇总，敞口使用横向条而非大号环图。
- 风险：回撤图明确 5% 预警与 7% 限制，并用 Risk Ledger 展示安全拦截、错过和自动终止记录。
- 复盘：关闭过程、已写回/未兑现结果、归因线索和只读的“自动校准”。
