...
- 顶部市场条集中展示所选市场、快照新鲜度、账户模式、当前收益、目标差、机会、可跟进项、持仓和最大回撤。
- `WorkbenchViewModel` 是首页与主题页的统一派生边界；标题收益和收益曲线最后一点使用同一个所选视图，不能各自选择不同账户或市场口径。
- 主工作区由收益图、当前审阅右轨、压缩机会漏斗和底部 blotter 组成。blotter 分为 `当前机会 / 持仓 / 已完成 / 待复盘`；成交、错过和取消等终态不得回退到当前机会。
- 选择 `实盘` 时进入独立的 `实盘待接入` 门禁，只说明账户授权、风险校验和成交回执要求，不展示真实资金或任何买卖、下单、撤单、确认交易控件。
- 收益归因缺少明确非零贡献时显示真实空状态；后台 runtime code 统一翻译为用户文案，不原样展示下划线错误码。

本阶段只验收 1280×720 与 1440×900 桌面视口。移动端导航、触控和手机布局明确延期；现有快照 API 契约没有变化。
