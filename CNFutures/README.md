...
`session_windows` 使用分钟边界；普通结束分钟仅允许该分钟的 `:00.000000` 端点。`1440` 专门表示跨午夜的 `24:00` seam，采用半开边界，因此 `[1260, 1440]` 连续覆盖 `21:00:00` 至 `23:59:59.999999`，再由 `[0, 60]` 覆盖至精确 `01:00:00`；它不等同于真实的 `23:59` 收盘。
