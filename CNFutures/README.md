...
该 one-shot 的唯一 availability source 是 query envelope 的 `metadata.observed_at`：每个 query 还须有带时区的 `metadata.data_through`，并满足 `data_through <= observed_at <= decision_time`；bars 进一步要求 `bar_time <= data_through`、整 5 分钟网格且位于 TD 交接的显式 session windows。row `available_at` 不是 provider-native knowledge-time，不能放宽 PIT。每个 role profile 还须携带共享 `dataset_contract_fingerprint` 生成的预期指纹；验收器按 catalog 的七项公开合同字段（ID、schema、fields、operators、order、limits、identity）重算并精确匹配，query 的 filter/operator、sort 与 identity fields 也只能引用 catalog 声明。readback fixture 保留 canonical filter/sort/identity/cursor identity，并将 TD list/delist 事实投影为按 trade date 的 tradeability；它不假定 TD 原始字段名。通过完整 handoff 的结果才会按共享 readiness 合同标记 `readiness.observation_ready=true`；`historical_pit_ready=false`、`delayed_paper_ready=false` 和 `execution_ready=false` 始终保持，直到另有 first-seen/revision-vintage、历史 PIT、资本和 reconcile 证据。它只输出 non-authoritative `observation`、`hold` 或 `risk_reject`，且固定 `learning_evidence_eligible=false`，不会连接 18082、启动 delayed-paper 或形成 fill。真实市场数据的 read-only parity 通过，仍不等于 runtime 或 delayed-paper GO。
