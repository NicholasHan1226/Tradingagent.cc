...
def _style_metric(
    *,
    ...
    filled_count = int(
        style_state.get("filled_count") or style_score.get("filled_count") or 0
    )
    ...
    diagnostic_ratio = realized_pnl / max(1.0, drawdown + fee) if trade_count else None
    forward_state = forward_state if isinstance(forward_state, dict) else {}
    threshold_candidate = (
        threshold_candidate if isinstance(threshold_candidate, dict) else {}
    )
    total_pnl = round(realized_pnl + unrealized_pnl, 6)
    observation_count = int(style_state.get("observation_count") or 0)
    completed_round_trips = int(style_score.get("completed_round_trip_count") or 0)
    candidate_pool_diagnostic_eligible = (
        style_score.get("status") == "eligible_for_candidate_pool"
        and completed_round_trips > 0
    )
    status = style_state.get("status") or style_score.get("status") or "observe"
    if trade_count <= 0 and observation_count > 0:
        status = "observe"
    return {
        ...
        # No same-frequency net-return series is available in this review
        # projection.  The former PnL/(drawdown+fee) value is retained only
        # under an explicit diagnostic name and must never be consumed as
        # Sharpe, DSR, or promotion evidence.
        "sharpe": None,
        "sharpe_status": "unavailable_no_same_frequency_net_return_series",
        "net_pnl_to_drawdown_plus_fee_ratio": (
            round(diagnostic_ratio, 6) if diagnostic_ratio is not None else None
        ),
        "diagnostic_ratio_only": True,
        "dsr_eligible": False,
        "dsr_status": "unavailable_sharpe_missing",
        "promotion_metric_eligible": False,
        "candidate_pool_diagnostic_eligible": candidate_pool_diagnostic_eligible,
        "trades": trade_count,
        "avg_hold_hours": 0.0,
        "status": status,
        ...
        "hold_count": int(style_state.get("hold_count") or 0),
        "risk_rejection_count": int(style_state.get("risk_rejection_count") or 0),
...
def _write_non_promotional_style_performance(
    path: Path,
    metrics: list[dict[str, Any]],
) -> None:
    """Upsert CN display rows without invoking Sharpe-based style ranking.

    This file is a dashboard projection, not an evolution authority.  Keeping
    the explicit null Sharpe/status fields prevents the generic historical
    tracker from coercing unavailable evidence to ``0.0`` and ranking it as if
    a return-series statistic had been calculated.
    """
    ...
    replacement_keys = {
        (
            str(row.get("market") or STYLE_REVIEW_MARKET),
    ...
    rows = [*kept, *(dict(row) for row in metrics)]
...
def write_style_outputs(
    ...
    *,
    ...
    score_summary = (
        payload.get("score_summary")
        if isinstance(payload.get("score_summary"), dict)
        else {}
    )
    style_scores = (
        score_summary.get("style_scores")
        if isinstance(score_summary.get("style_scores"), dict)
        else {}
    )
    health = (
        payload.get("style_health")
        if isinstance(payload.get("style_health"), dict)
        else {}
    )
    styles = payload.get("styles") if isinstance(payload.get("styles"), dict) else {}
    forward_summary = (
        payload.get("forward_label_summary")
        if isinstance(payload.get("forward_label_summary"), dict)
        else {}
    )
    forward_styles = (
        forward_summary.get("styles")
    ...
    hold_summary = (
        payload.get("hold_reason_summary")
        if isinstance(payload.get("hold_reason_summary"), dict)
        else {}
    )
    hold_styles = (
        hold_summary.get("by_style")
    ...
    threshold_candidates = (
        payload.get("dynamic_threshold_candidates")
        if isinstance(payload.get("dynamic_threshold_candidates"), list)
        else []
    )
    threshold_by_style = {
    ...
    if isinstance(position_pnl_summary, dict):
        for style_name, values in position_pnl_summary.items():
            if isinstance(values, dict):
                unrealized_by_style[style_name] = _safe_float(
    ...
    metrics = [
        _style_metric(
            ...
            style_name=style_name,
            style_score=style_scores.get(style_name)
            if isinstance(style_scores.get(style_name), dict)
            else {},
            style_state=health.get(style_name)
            if isinstance(health.get(style_name), dict)
            else {},
            forward_state=forward_styles.get(style_name)
            if isinstance(forward_styles.get(style_name), dict)
            else {},
            threshold_candidate=threshold_by_style.get(style_name)
            if isinstance(threshold_by_style.get(style_name), dict)
            else {},
            unrealized_pnl=unrealized_by_style.get(style_name, 0.0),
        )
        for style_name in style_names
    ]
    economic_style_names = {
        str(metric["style_name"])
        for metric in metrics
        if int(metric.get("trades") or 0) > 0
        or int(metric.get("filled_count") or 0) > 0
        or int(metric.get("completed_round_trip_count") or 0) > 0
    }
    if economic_style_names:
        _write_non_promotional_style_performance(
            output_dir / "style_performance.jsonl",
            [
                metric
                for metric in metrics
                if str(metric["style_name"]) in economic_style_names
            ],
        )
    style_comparison = [
        {
    ...
    output = {
        ...
        "style_states": [
            {
                "style_name": style_name,
                "status": (health.get(style_name) or {}).get("status", "observe")
                if isinstance(health.get(style_name), dict)
                else "observe",
                "suggested_action": (health.get(style_name) or {}).get(
                    "suggested_action", ""
                )
                if isinstance(health.get(style_name), dict)
                else "",
        ...
        if isinstance(payload.get("error_summary"), dict)
        else {},
        "hold_count": int(payload.get("hold_count") or 0),
        "hold_reason_summary": payload.get("hold_reason_summary")
        if isinstance(payload.get("hold_reason_summary"), dict)
        else {},
        ...
        if isinstance(payload.get("affordability"), dict)
        else {},
...
def append_review(
    *,
    ...
    payload["style_output_paths"] = write_style_outputs(
        payload, review_path=target, position_pnl_summary=position_pnl_summary
    )
