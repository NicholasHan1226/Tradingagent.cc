...
class SimCapitalTest(unittest.TestCase):
    def test_us_crypto_pm_default_to_10000_base_currency(self) -> None:
        for market in ("us", "crypto", "pm"):
            ...
            self.assertAlmostEqual(
                capital * fx_to_cny(market), 10_000.0 * fx_to_cny(market), places=2
            )
    ...
    def test_illegal_capital_fails_to_50k_not_200k(self) -> None:
        ...
        self.assertEqual(
            default_sim_capital("cn_futures", capital_cny=30_000.0), 50_000.0
        )
        with patch.dict(os.environ, {"ASHARE_SIM_CAPITAL_TIER": "12345"}, clear=False):
            self.assertEqual(default_sim_capital("ashare"), 50_000.0)
        with patch.dict(
            os.environ, {"CN_FUTURES_SIM_CAPITAL_TIER": "999999"}, clear=False
        ):
            self.assertEqual(default_sim_capital("cn_futures"), 50_000.0)
    ...
    def test_cn_futures_tier_env_cannot_override_production_capital(self) -> None:
        with patch.dict(
            os.environ, {"CN_FUTURES_SIM_CAPITAL_TIER": "200000"}, clear=False
        ):
            self.assertEqual(default_sim_capital("cn_futures"), 50_000.0)
    ...
    def test_a_share_and_cn_futures_explicit_capital_cannot_create_parallel_tier(
        self,
    ) -> None:
        self.assertEqual(default_sim_capital("ashare", capital_cny=50_000.0), 50_000.0)
        self.assertEqual(default_sim_capital("ashare", capital_cny=100_000.0), 50_000.0)
        self.assertEqual(default_sim_capital("ashare", tier="200000"), 50_000.0)
        self.assertEqual(
            default_sim_capital("cn_futures", capital_cny=200_000.0), 50_000.0
        )
        self.assertEqual(default_sim_capital("cn_futures", tier="100000"), 50_000.0)

    def test_invalid_capital_cny_parameter_falls_back_to_default(self) -> None:
        self.assertEqual(
            default_sim_capital("ashare", capital_cny=30_000.0), DEFAULT_SIM_CAPITAL_CNY
        )
        self.assertEqual(
            default_sim_capital("cn_futures", capital_cny=150_000.0),
            DEFAULT_SIM_CAPITAL_CNY,
        )
    ...
    def test_only_one_canonical_cny_tier_is_exported(self) -> None:
        from shared.markets.sim_capital import ALLOWED_CNY_TIERS
