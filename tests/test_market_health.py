...
class MarketHealthTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.ashare_execution_root = (
            self.root / "shared/logs/execution_lineages" / ASHARE_EXECUTION_LINEAGE_ID
        )
    ...
    def _write_ashare_execution_json(self, name: str, payload: object) -> None:
        path = self.ashare_execution_root / name
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(json.dumps(payload, ensure_ascii=False), encoding="utf-8")
    ...
    def test_signal_queue_isolation_fails_when_shadow_leaks_into_execution_pending(
        self,
    ) -> None:
        self._write_json(
            "signals/pending/SHADOW-ashare-000001.json",
            {"capital_layer": "shadow", "order_id": "SHADOW-1"},
        )
        ...
        self.assertIn(
            "signals/pending/SHADOW-ashare-000001.json",
            check.details["leaked_shadow_sample"],
        )
    ...
    def test_signal_queue_isolation_reports_expired_execution_pending(self) -> None:
        ...
        self.assertEqual(check.status, "fail")
        self.assertEqual(
            check.details["stale_execution_sample"][0]["reason"], "valid_until_expired"
        )

    def test_signal_queue_isolation_passes_when_shadow_uses_shadow_subqueue(
        self,
    ) -> None:
        self._write_json(
            "signals/shadow/pending/SHADOW-ashare-000001.json",
            {"capital_layer": "shadow", "order_id": "SHADOW-1"},
        )
    ...
    def test_shadow_ledger_passes_when_no_shadow_trades_exist(self) -> None:
        ...
        self.assertEqual(
            check.details["ashare_pnl"]["valuation_source"], "shadow_broker_replay"
        )

    def test_shadow_ledger_detects_invalid_ashare_codes_and_missing_pnl_fields(
        self,
    ) -> None:
        self._write_json(
            "shared/logs/shadow/shadow_pnl.json",
            {"ashare_shadow": {"positions": {"200011.SZ": {}}}},
        )
        self._write_json(
            "shared/logs/shadow/shadow_positions.json",
            {"ashare_shadow": {"200011.SZ": {}}},
        )
        ...
        self.assertEqual(check.status, "fail")
        self.assertGreater(
            check.details["invalid_ashare_code_matches"][
                "shared/logs/shadow/shadow_pnl.json"
            ],
            0,
        )
        ...
        self._write_json(
            "shared/logs/shadow/shadow_pnl.json",
            {
                "ashare_shadow": {
                    ...
                    "market_value": 100,
                    "total_pnl": 1,
                    "valuation_source": "unit",
                }
            },
        )
        self._write_json(
            "shared/logs/shadow/shadow_positions.json",
            {"ashare_shadow": {"000001.SZ": {}}},
        )
        path = self.root / "shared/logs/shadow/shadow_trades.jsonl"
        path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_sim_position_sync_reads_bootstrap_snapshot_state(self) -> None:
        self._write_ashare_execution_json(
    ...
    def test_failure_receipts_warn_when_failed_signal_has_no_receipt(self) -> None:
        self._write_json(
            "signals/failed/ORDER-1.json", {"order_id": "ORDER-1", "status": "failed"}
        )
    ...
    def test_failure_receipts_passes_when_only_after_hours_validation_samples_exist(
        self,
    ) -> None:
        trades = self.ashare_execution_root / "local_sim_trades.jsonl"
        ...
        self.assertEqual(
            check.details["sample_quality"]["by_reason"],
    ...
    def test_local_sim_ledger_checks_code_field_not_raw_line_text(self) -> None:
        ...
        trades_path = self.ashare_execution_root / "local_sim_trades.jsonl"
        positions_path = self.ashare_execution_root / "local_sim_positions.json"
        pnl_path = self.ashare_execution_root / "local_sim_pnl.json"
        snapshot_path = self.ashare_execution_root / "simulated_ashare_positions.json"
        trades_path.parent.mkdir(parents=True, exist_ok=True)
        trades_path.write_text(
            json.dumps(
                {"ts_code": "000001.SZ", "net_amount": 2000.0}, ensure_ascii=False
            )
            + "\n",
            encoding="utf-8",
        )
        positions_path.write_text(
            json.dumps({"ashare_sim": {}}, ensure_ascii=False), encoding="utf-8"
        )
        snapshot_path.parent.mkdir(parents=True, exist_ok=True)
        snapshot_path.write_text(
            json.dumps(
                {"positions": [], "pnl": {"ashare_sim": {"cash_available": 50000}}},
                ensure_ascii=False,
            ),
            encoding="utf-8",
        )
        pnl_path.write_text(
            json.dumps(
                {"ashare_sim": {"cash_available": 50000, "positions": {}}},
                ensure_ascii=False,
            ),
            encoding="utf-8",
        )
        ...
        with patch.object(local_sim_ledger, "LOCAL_SIM_TRADES", trades_path):
            with patch.object(local_sim_ledger, "LOCAL_SIM_POSITIONS", positions_path):
                with patch.object(local_sim_ledger, "LOCAL_SIM_PNL", pnl_path):
                    with patch.object(
                        local_sim_ledger, "LOCAL_SIM_POSITIONS_SNAPSHOT", snapshot_path
                    ):
    ...
    def test_local_sim_ledger_fails_when_snapshot_and_pnl_disagree(self) -> None:
        ...
        local_dir = self.ashare_execution_root
        ...
        snapshot_path = self.ashare_execution_root / "simulated_ashare_positions.json"
        trades_path.write_text(
            json.dumps({"ts_code": "000001.SZ"}, ensure_ascii=False) + "\n",
            encoding="utf-8",
        )
        positions_path.write_text(
            json.dumps(
                {"ashare_sim": {"000001.SZ": {"quantity": 100}}}, ensure_ascii=False
            ),
            encoding="utf-8",
        )
        pnl_path.write_text(
        ...
        self._write_ashare_execution_json(
            "simulated_ashare_positions.json",
            {"positions": [], "pnl": {"ashare_sim": {"cash_available": 190000}}},
        )
    ...
    def test_local_sim_ledger_passes_when_after_hours_trade_is_isolated_validation_sample(
        self,
    ) -> None:
        from shared.execution import local_sim_ledger
        ...
        self._write_ashare_execution_json(
            "simulated_ashare_positions.json",
            {
                "positions": [{"ts_code": "000001.SZ"}],
                "pnl": {"ashare_sim": {"cash_available": 190000}},
            },
        )
        ...
        self.assertEqual(
            check.details["sample_quality"]["strategy_sample_valid_count"], 0
        )
    ...
    def test_local_sim_ledger_warns_when_provenance_is_missing(self) -> None:
        ...
        self.assertEqual(
            check.details["sample_quality"]["by_reason"],
            {"missing_ashare_candidate_provenance": 1},
        )

    def test_capital_plan_alignment_fails_when_plan_misses_snapshot_positions(
        self,
    ) -> None:
        trades = self.ashare_execution_root / "local_sim_trades.jsonl"
        trades.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_capital_plan_alignment_warns_when_plan_is_older_than_snapshot(
        self,
    ) -> None:
        trades = self.ashare_execution_root / "local_sim_trades.jsonl"
        trades.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_capital_plan_alignment_passes_when_stale_plan_only_reflects_validation_samples(
        self,
    ) -> None:
        trades = self.ashare_execution_root / "local_sim_trades.jsonl"
        trades.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_cn_futures_sim_market_loop_reads_governed_wrapper_log(self) -> None:
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                return_value={"in_session": True, "samples_expected_today": True},
            ):
                check = market_health._check_sim_market_loop(
                    "cn_futures", "job_cn_futures_sim.sh"
                )

        self.assertEqual(
            check.details["latest_cron_result"]["path"],
            "shared/logs/cron/job_cn_futures_sim.log",
        )
        self.assertEqual(
            check.details["latest_cron_result"]["payload"]["hold_count"], 3
        )

    def test_ashare_sim_market_loop_reads_governed_wrapper_log(self) -> None:
        log = self.root / "shared/logs/cron/job_ashare_sim_exec.log"
        log.parent.mkdir(parents=True)
        log.write_text(
            'noise\n{"market":"ashare","state":"ok","order_count":0}\n',
            encoding="utf-8",
        )
        ...
        with patch.object(
            ...
            with patch.object(
                market_health,
                "_market_session_state",
                return_value={"in_session": False, "samples_expected_today": False},
            ):
                check = market_health._check_sim_market_loop(
        ...
        self.assertEqual(
            check.details["latest_cron_result"]["path"],
            "shared/logs/cron/job_ashare_sim_exec.log",
        )
        self.assertEqual(
    ...
    def test_crypto_sim_market_loop_observes_when_momentum_threshold_not_met(
        ...
        self.assertEqual(check.details["fail_reasons"], [])
        self.assertIn(
            "crypto_waiting_for_momentum_signal", check.details["warn_reasons"]
        )
        self.assertNotIn("market_data_degraded", check.details["warn_reasons"])
        self.assertEqual(check.details["diagnostic_class"], "strategy_wait")
        self.assertEqual(check.details["execution_fault"], False)

    def test_crypto_reader_degraded_cron_payload_cannot_pass_as_strategy_wait(
        self,
    ) -> None:
        with (
            patch.object(
    ...
    def test_crypto_reader_degraded_cron_payload_overrides_clean_probe(self) -> None:
        ledger = (
            self.root / "shared/logs/sim_ledger/crypto/aggressive/trade_journal.jsonl"
        )
        ledger.parent.mkdir(parents=True)
        ledger.write_text(
    ...
    def test_crypto_sim_market_loop_marks_strategy_wait_even_with_existing_ledger(
        self,
    ) -> None:
        ledger = (
            self.root / "shared/logs/sim_ledger/crypto/aggressive/trade_journal.jsonl"
    ...
    def test_crypto_sim_market_loop_fails_when_candidate_exists_but_no_ledger(
        self,
    ) -> None:
        with patch.object(
            market_health,
    ...
    def test_ashare_sim_loop_warns_without_production_trade_sample(self) -> None:
        with patch.object(
            market_health,
            "_probe_market_data",
            return_value={"status": "ok", "asset_count": 10},
        ):
            with patch.object(
                market_health,
                "_market_session_state",
        ...
        self.assertEqual(check.status, "warn")
        self.assertIn(
            "server_local_sim_has_no_production_trades_yet",
            check.details["warn_reasons"],
        )

    def test_ashare_sim_loop_observes_when_no_portfolio_orders_explains_no_trade(
        self,
    ) -> None:
            ...
            json.dumps(
                {
                    ...
                    "no_trade_explanation": {
                        ...
                        "candidate_decision_trace": [
                            {
                                "symbol": "AAA",
                                "drop_reason": "capital_plan_capacity_zero",
                            }
                        ],
        ...
        self.assertEqual(check.details["diagnostic_class"], "strategy_wait")
        self.assertEqual(
            check.details["no_trade_explanation"]["category"], "no_portfolio_orders"
        )

    def test_ashare_sim_loop_warns_when_no_portfolio_orders_lacks_trace_evidence(
        self,
    ) -> None:
        log_path = self.root / "shared/logs/ashare_no_trade_explanations.jsonl"
        log_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        self.assertEqual(
            check.details["no_trade_explanation"]["evidence_status"], "incomplete"
        )
        self.assertIn(
    ...
    def test_ashare_sim_loop_warns_when_empty_candidate_pool_lacks_plan_evidence(
        self,
    ) -> None:
        log_path = self.root / "shared/logs/ashare_no_trade_explanations.jsonl"
        log_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_ashare_sim_loop_accepts_empty_candidate_pool_with_plan_evidence(
        self,
    ) -> None:
        log_path = self.root / "shared/logs/ashare_no_trade_explanations.jsonl"
        log_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        self.assertEqual(
            check.details["no_trade_explanation"]["evidence_status"], "ready"
        )

    def test_ashare_sim_loop_warns_when_risk_rejection_gap_lacks_trace_evidence(
        self,
    ) -> None:
        log_path = self.root / "shared/logs/ashare_no_trade_explanations.jsonl"
        log_path.parent.mkdir(parents=True, exist_ok=True)
            ...
            json.dumps(
                {
                    ...
                    "no_trade_explanation": {
                        ...
                        "sample_risk_rejections": [
                            {"symbol": "AAA", "reasons": ["unit risk"]}
                        ],
                    },
                },
                ensure_ascii=False,
    ...
    def test_ashare_sim_loop_uses_today_trade_rows_not_historical_total(self) -> None:
        local_sim = self.ashare_execution_root / "local_sim_trades.jsonl"
        local_sim.parent.mkdir(parents=True, exist_ok=True)
        local_sim.write_text(
            json.dumps(
                {"trade_date": "20260707", "ts_code": "600000.SH", "status": "filled"}
            )
            + "\n",
            encoding="utf-8",
        )
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                return_value={
                    "in_session": False,
                    "samples_expected_today": True,
                    "local_time": "2026-07-08T15:30:00+08:00",
                },
            ):
                check = market_health._check_sim_market_loop(
                    "ashare", "job_ashare_sim_exec.sh"
            ...
            json.dumps(
                {
                    ...
                    "no_trade_explanation": {
                        ...
                        "counts": {"candidates": 3, "orders": 0},
                        "candidate_decision_trace": [
                            {"symbol": "600000.SH", "decision": "skip"}
                        ],
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                return_value={
                    "in_session": True,
                    "samples_expected_today": True,
                    "local_time": "2026-07-08T10:30:00+08:00",
                },
            ):
                check = market_health._check_sim_market_loop(
                    "ashare", "job_ashare_sim_exec.sh"
    ...
    def test_default_sim_market_health_excludes_deferred_hk(self) -> None:
        ...
        with patch.object(
            market_health, "_installed_crontab_text", return_value=("", "")
        ):
            with patch.object(
                market_health, "_check_sim_market_loop", side_effect=fake_check
            ):
    ...
    def test_cn_futures_sim_loop_warns_without_live_samples(self) -> None:
        with patch.object(
            market_health,
            "_probe_market_data",
            return_value={"status": "fail", "reason": "futures_universe_missing"},
        ):
            with patch.object(
                market_health,
                "_market_session_state",
        ...
        self.assertIn(
            "cn_futures_review_has_no_samples_yet", check.details["warn_reasons"]
        )
    ...
    def test_cn_futures_sim_loop_waits_without_sample_before_session(self) -> None:
        with patch.object(
            ...
            return_value={
                "status": "ok",
                "reason": "futures_intraday_waiting_for_next_session",
            },
        ):
            with patch.object(
                market_health,
    ...
    def test_cn_futures_sim_loop_accepts_coverage_observation_state(self) -> None:
        ...
        with (
            ...
            patch.object(
                market_health, "_latest_cron_result", return_value=cron_result
            ),
        ):
            check = market_health._check_sim_market_loop(
                "cn_futures", "job_cn_futures_sim.sh", ""
        ...
        self.assertEqual(check.status, "pass")
        self.assertNotIn(
            "latest_cron_status=observation_only", check.details["warn_reasons"]
        )
    ...
    def test_cn_futures_sim_loop_reads_append_only_review_as_ledger(self) -> None:
        ...
        with patch("CNFutures.session.active_trade_date", return_value="20260710"):
            with patch.object(
                market_health,
                "_probe_market_data",
                return_value={"status": "warn", "priced_signal_count": 0},
            ):
                with patch.object(
                    market_health,
                    "_market_session_state",
        ...
        self.assertEqual(
            check.details["ledger"]["latest_style_health"]["trend"]["status"],
            "active_sample",
        )
    ...
    def test_cn_futures_market_health_entrypoint_is_read_only(self) -> None:
        ...
        with patch(
            "shared.runtime_test.cn_futures_live_check.run_live_check",
