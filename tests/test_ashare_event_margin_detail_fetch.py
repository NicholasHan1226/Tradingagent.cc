"""Offline tests for the all-market margin_detail daily fetcher."""
...
from Ashare import event_margin_detail_fetch as fetch
...
class SessionDaysTest(unittest.TestCase):
    ...
    def test_missing_index_cache_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaises(fetch.MarginDetailFetchError):
...
class FetchSweepTest(unittest.TestCase):
    ...
    def test_fetches_writes_and_is_idempotent(self) -> None:
        ...
        def fake_call_api(api: str, params: dict):
            ...
            return ["trade_date", "ts_code", "rzye"], [
                [params["trade_date"], "600000.SH", "100.0"]
            ]
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch(
                ...
                summary = fetch.fetch_margin_detail_daily(cache)
            self.assertEqual(calls == SESSIONS or len(calls) >= 4, True)
            self.assertEqual(summary["fetched"] + summary["skipped_existing"],
                             len(SESSIONS))
            day_path = cache / "margin_detail_daily" / f"{SESSIONS[0]}.csv"
            with day_path.open(encoding="utf-8") as handle:
                rows = list(csv.reader(handle))
            self.assertEqual(rows[0], ["trade_date", "ts_code", "rzye"])
            # Rerun hits no network at all.
            calls.clear()
            with unittest.mock.patch(
                "Ashare.event_calendar_fetch.call_api", side_effect=fake_call_api
            ):
                again = fetch.fetch_margin_detail_daily(cache)
    ...
    def test_empty_and_failed_days_leave_no_file(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch(
                ...
                summary = fetch.fetch_margin_detail_daily(cache)
            out_dir = cache / "margin_detail_daily"
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                with unittest.mock.patch(
                    ...
                    with self.assertRaises(fetch.MarginDetailFetchError):
                        fetch.fetch_margin_detail_daily(cache)
            finally:
