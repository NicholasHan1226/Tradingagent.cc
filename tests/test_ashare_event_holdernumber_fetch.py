"""Offline tests for the per-symbol stk_holdernumber fetcher."""
...
from Ashare import event_holdernumber_fetch as fetch
...
class StemsTest(unittest.TestCase):
    def test_stems_come_from_daily_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _write_csv(cache / "daily_600000SH.csv",
                       ["trade_date", "close"], [["20260105", 10.0]])
            _write_csv(cache / "holdernum_000001SZ.csv",
    ...
    def test_missing_daily_cache_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaisesRegex(fetch.HolderNumFetchError,
                                        "daily_cache_missing"):
                fetch.fetch_holdernumber(Path(tmp))
...
class FetchSweepTest(unittest.TestCase):
    ...
    def test_fetches_per_symbol_writes_and_is_idempotent(self) -> None:
        ...
        def fake_call(api: str, params: dict):
            self.assertEqual(api, "stk_holdernumber")
            calls.append(dict(params))
            return (
                list(FIELDS),
                [["000001.SZ", "20260815", "20260630", 450712]],
            )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch(
                ...
                summary = fetch.fetch_holdernumber(cache, delay_seconds=0.0)
            ...
            # sweep contract uniform with #462/#465: ts_code only
            self.assertNotIn("start_date", calls[0])
            self.assertNotIn("end_date", calls[0])
            target = cache / "holdernum_000001SZ.csv"
            self.assertTrue(target.exists())
            with target.open(newline="", encoding="utf-8") as handle:
                written = list(csv.reader(handle))
            self.assertEqual(written[0], FIELDS)
            self.assertEqual(written[1][3], "450712")
            # idempotent rerun: everything skipped, no extra calls
            before = len(calls)
            with unittest.mock.patch(
                "Ashare.event_calendar_fetch.call_api", fake_call
            ):
                again = fetch.fetch_holdernumber(cache, delay_seconds=0.0)
    ...
    def test_empty_and_failed_symbols_leave_no_file(self) -> None:
        def fake_call(api: str, params: dict):
            ...
            return (list(FIELDS), [])  # no disclosure record: empty
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse((cache / "holdernum_000001SZ.csv").exists())
            self.assertFalse((cache / "holdernum_600000SH.csv").exists())
    ...
    def test_disk_guard_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch(
                ...
                with self.assertRaisesRegex(fetch.HolderNumFetchError,
                                            "disk_low"):
                    fetch.fetch_holdernumber(cache, delay_seconds=0.0)
