...
- `data/`：mock-first `GET /v1/catalog` + `POST /v1/query` provider-neutral client/Evidence Gate；base URL、catalog version、dataset IDs 与 policy 显式配置。当前不证明生产 SS runtime，禁止直读兄弟仓文件、旧专用端点或 provider fallback。
