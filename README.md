...
- 可执行自动闭环只在网络关闭的冻结 fixture 中得到验证；相同输入的业务 bundle 已验证不受本机输出根绝对路径影响，同根 replay 不重放 transport。另有 canonical-capital composition 的测试候选证明单一模拟账本、由预声明确定性策略选择 simulation Champion、动态generation/lineage、capital outbox与reconcile可组合，但它还没有 CLI/scheduler/live sample。真实 TradingDatas V1、市场日历 scheduler 和持续自然运行尚未验收。旧四风格、exploration/exploitation 路径仍是 time-boxed legacy，不是 V1 当前路由。
...
288 槽只约束其命名运行成熟度及后续晋级/风险能力；一个 receipt-bound、PIT 安全、
局部连续且不跨 gap 的完整 symbol/shard/segment 可以先进入确定性模拟与结果积累。
该准入本身不能扩风险、获得执行权限或证明 edge。
真实执行角色在当前合同中固定关闭。
...
| Phase 3 | 将现有三风格shadow合同升级为统一 A股 50k CNY 候选路由 | 分组消融、独立收益来源、费用后增量、abstain价值、尾部与相关性稳定；只允许确定性 simulation/shadow 晋降级 |
| Phase 4 | live-readiness dossier 与受控试运行 charter | 账户、资本、订单和真实执行另行授权；不由 simulation 晋级推导 |
