...
> **⚠️ 变更后必须更新本文件。**
>
> 最后更新：2026-07-07 (多市场模拟资金 20 万人民币口径、收益看板去重与维护样本隔离)
...
- **多市场信号门禁**：2026-07-07 起 Crypto/US/PM/HK 通用 `run_sim.py` 不再把 SharedSignals 行情行直接转换成 `buy` 模拟信号；只有输入行显式带 `side/action/direction/signal/decision/recommendation=buy|sell` 时才会进入 StyleRunner。人工价格行烟测必须显式设置 `TRADINGAGENT_SIM_ALLOW_PRICE_ONLY_SIGNALS=1`，且这类样本自动写入 `exclude_from_dashboard=true` 与 `sample_type=price_only_smoke`，不进入生产看板/复盘/演化口径。
