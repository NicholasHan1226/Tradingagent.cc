...
> **⚠️ 变更后必须更新本文件。**
>
> 最后更新：2026-07-06 (A-share rebalance and capital-plan audit trail)
...
- [x] `front/` 首页交易漏斗已改为事件优先的动态管道展示：机会进入、初筛、研究、风控、待执行和结果分流在同一面板内流动展示；该面板只读 `funnelEvents` / signals，不写队列、不触发执行。
