...
- **前端自动化观测台已发布（2026-07-11）**：前端发布代码为 `33f47fd`，本条所在 STATUS-only 提交作为最终发布记录同步到三端。`front/` 已重构为 Hyperliquid 风格的全自动系统只读观测台，主导航为 `总览 / 收益 / 过程 / 持仓 / 风险 / 复盘`；右轨、五阶段过程带和 `运行中 / 持仓 / 已完成 / 自动复盘` blotter 只展示自动运行过程与结果。生产源码路径 `/opt/investment/tradingagent` 已从 GitHub `main` 快进；静态前端与只读 API 均使用 Node `v24.4.1` 构建，`tradingagent-front-api.service=active`，内部 `/healthz` 返回 `{"ok":true,"service":"trading-agent-snapshot-api"}`。公开 `https://dashboard.tradingagent.cc/` 返回 HTTP 200，公开快照返回 `mode=simulated` 及 performance/signals/holdings/decisions/risk 五个域；浏览器实测 build=`20260711-automated-observatory`，1440×900 和 1280×720 无横向溢出、无控制台错误/告警，六页导航与实盘只读门禁通过，页面没有人工决策文案或买卖/下单/撤单/确认交易控件。发布验证为 25 个测试文件/130 项测试、lint、前端构建、API 构建全部通过；回滚副本戳 `20260711040445`。本次未改变快照 API 契约、交易队列、资金、策略、cron、执行器或真实资金行为；移动端继续延期。
