...
- **多市场信号门禁**：2026-07-07 起 Crypto/US/PM/HK 通用 `run_sim.py` 不再把 SharedSignals 行情行直接转换成 `buy` 模拟信号；输入行显式带 `side/action/direction/signal/decision/recommendation=buy|sell`，或由市场专属策略生成 `signal_source=explicit_strategy_signal` 后，才会进入 StyleRunner。当前内置策略口径为 Crypto 动量突破、US 趋势跟随、PM 模型概率相对市场概率价差；价格行只用于估值/成交价。人工价格行烟测必须显式设置 `TRADINGAGENT_SIM_ALLOW_PRICE_ONLY_SIGNALS=1`，且这类样本自动写入 `exclude_from_dashboard=true` 与 `sample_type=price_only_smoke`，不进入生产看板/复盘/演化口径。
