...
- **多市场**：Crypto/US/HK 通过 symlink 复用旧系统代码，新架构代码尚未独立
...
- 多市场代码依赖旧系统 symlink，未独立实现
- 集合竞价支持标记为 STUB，未实现
- A 股实盘路径仍是人工（模拟盘信号 → 邮件发用户 → 手动执行）

## 三、下一步

1. [ ] Crypto/US/HK/PM 模拟盘闭环（各自独立实现，不依赖旧系统 symlink）
2. [ ] 多市场代码从 symlink 迁移为独立实现（消除对旧系统的运行时依赖）
