...
> **⚠️ 变更后必须更新本文件。**
>
> 最后更新：2026-07-06 (stable dashboard equity aggregation)
...
- **A股开盘验收与无交易分层**：`ashare_opening_validator` 的 first-sample 报告已把 5分钟 bar、信号状态、服务器本地模拟成交、签名回执、复盘行数和 no-trade 分类汇总到同一报告；若当天没有交易，会优先读取最新 `shared/logs/ashare_no_trade_explanations.jsonl`，把无候选、无信号卡、风控全拒、资金/组合构建阻塞、重复幂等、执行跳过、执行失败、回执缺失和复盘待生成区分开。`opening_acceptance.py` 短文本同步展示 bar/信号/成交/回执/复盘计数。
