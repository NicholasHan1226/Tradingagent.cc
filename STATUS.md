...
- **A股 epoch-2 派生复盘 authority 质量门已补齐（2026-07-11，本地待集成）**：`evolution_controller` 的 build/write 现在必须用持久化 epoch、50,000 CNY 本金和原始 cutover 字符串精确校验 portfolio evidence，错误证据只可观察且 writer 在创建 latest/log 前拒绝；`sample_learning` 对每条成交逐行执行同一 authority 校验，错误/缺失本金或 cutover 的行不会进入质量、探索预算或因子统计，并保留分原因计数；epoch reset apply 会在首个 mutation 前复核 plan 阶段缺失路径，并以 no-replace 原子创建 bootstrap，晚到 writer 返回 `blocked / plan_stale` 且不覆盖或删除其文件。三个反例先 RED 后 GREEN，Task3 与相关 writers 回归 `178 passed`；尚未 push、deploy 或执行生产 reset。
