...
- **复盘与自我演化验收**：2026-07-09 修复 A股 server-local simulated 成交进入日报复盘时的字段传递口径，复盘读取 local_sim 时必须保留 `filled_price/avg_price`、`trade_timestamp_bj`、`ashare_session_valid`、`fill_price_source`、`fill_price_source_class`、`fill_evidence`、`candidate_pool_layer`、`execution_source` 等样本判断字段；`fill_price_source=signal_card.price` 且有真实成交价的 A股 server-local 成交可作为明确成交价来源，缺成交价仍只作链路验证样本。新增 `shared.runtime_test.self_evolution_health`，只读检查各市场账本策略样本是否被演化层看到、风格权重 action 与最终 overlay 是否一致、是否有正向演化证据；该检查已接入 `full_acceptance --profile prod`，发现复盘/演化断链时给 warn，不直接阻断交易闭环。追加 `Ashare/portfolio_evolution.py` 作为 A股组合级演化证据入口，每次 A股模拟调度成功后刷新 `shared/review/ashare/portfolio_evolution_latest.json` 和 append-only log；2026-07-09 起该入口会先用同一批 SharedSignals 盯市价刷新 `shared/logs/local_sim/local_sim_pnl.json` 与 `signals/positions/simulated_ashare_positions.json`，保证看板、健康检查和组合演化的 PnL 口径一致，同时保持成交事实 append-only 不变。健康检查会用该组合级证据判断 A股策略样本是否进入演化层，不再要求 A股伪造 aggressive/balanced 风格归因。追加 `Ashare/tier_experiments.py`，将 50,000 / 100,000 元作为独立资金档位实验账户，按各自本金、现金和 100 股手数 replay 主账户策略有效成交，写入独立账本与 `tier_experiments_latest.json`，并纳入组合级演化排名；2026-07-09 起升级为独立账户闭环：每个档位基于自身 replay 后的持仓与现金，调用按账户规模缩放的动态资金计划（`Ashare/capital_plan.py`），生成独立的 `capital_plan.json`，并在 `tier_experiments_latest.json` 和 `portfolio_evolution_latest.json` 的 `tier_experiments` 中暴露为复盘输入，不伪造 aggressive/balanced/conservative 风格归因。2026-07-10 起新增 `Ashare/evolution_controller.py` 自动进化控制器：每次组合演化后写 `evolution_decision_latest.json` / append-only log，把每日有效交易样本目标作为 simulated 硬门槛；当天未达标时输出 `force_sample_collection`，盘前 dry-run 与盘中资金计划会读取该决策并尝试受控探索仓，仍保留 candidate、价格、风控、现金、整手、T+1 和交易时段硬门禁。
...
- **模拟资金口径**：2026-07-08 起 `shared/markets/sim_capital.py` 是默认资金事实源。US/Crypto/PM 默认使用 10,000 USD/USDT/USDC 原币本金，并按当前 FX 折回 RMB 展示；A股与 CNFutures 默认使用 200,000 CNY，且支持 50,000 / 100,000 / 200,000 三档（`ASHARE_SIM_CAPITAL_TIER`、`CN_FUTURES_SIM_CAPITAL_TIER` 或 `default_sim_capital(..., capital_cny=...)`）。非法档位回退 200,000 CNY。多风格不是每个风格各给一份本金，而是在该市场总账户内按 active style weight 归一化拆分；看板不再把 US/Crypto/PM 的有效本金强行 floor 到 200,000 CNY，也不再让历史账本里的旧正数本金覆盖 10,000 美元规范本金；权益快照缺失而回退 `style_performance.jsonl` 时，US/Crypto/PM 的 PnL、realized/unrealized PnL 和 max drawdown 会先折算成人民币，再与人民币本金相除，避免原币 PnL / 人民币本金或人民币 PnL / 旧本金混算；A股/CNFutures 仍可保留真实人民币档位账本事实。
...
- **A股收益看板口径**：A股权益快照只接受 canonical `ashare/ashare_sim`，由 server-local `shared/logs/local_sim` 账本生成；该快照默认使用 `account_view=strategy_samples_only`，只展示策略有效样本的现金、持仓市值和 PnL；`audit_pnl` / `audit_positions_by_account` 仅用于追溯链路验证样本，不进入首页收益、交易表现或演化评分。旧 `ashare/<style>` 多风格测试账本不再进入 dashboard 汇总，避免 20k/16.6k 历史样本污染当前 200,000 元模拟盘口径。
...
- [x] `shared/runtime_test/opening_acceptance.py` 聚合验收已从泛化 API 健康兜底升级为业务 runtime evidence 兜底：当旧 SQLite 诊断不可用或日线诊断失败时，A股必须读取 `ashare_preopen_dry_run` 的数据/候选/资金/执行门禁摘要，CNFutures 必须读取 `cn_futures_live_check` 的 5 分钟数据/复盘/hold 摘要；runtime evidence 通过才降级为 pass，否则保留 warn/fail，避免午休/盘前窗口误报，也避免“API 绿灯但候选/执行证据为空”的误导。
...
- [x] 2026-07-09 追加优化 A股动态资金计划：新增 `sample_collection` 样本收集模式；当策略有效样本数低于最小样本数，或组合演化决策显示当日样本目标未达标，且候选质量达标、风控/数据无异常时，允许 1 笔 20,000-35,000 元探索仓继续积累有效样本，避免 2 个持仓 + 50,000 现金预留长期锁死交易；弱候选、高风险或数据异常仍保持防守/谨慎。`Ashare/evolution_controller.py` 会从组合演化结果写出 simulated-only `evolution_decision_latest.json`，主模拟循环和盘前 dry-run 均读取同一决策上下文，但最终订单仍必须通过 candidate、资金、T+1、时段和账本门禁。
- [x] 2026-07-10 追加 A股自动进化控制器：`Ashare/evolution_controller.py` 基于三账户 ranking、样本质量和 PnL 生成 simulated-only 下一步动作；每日有效交易样本目标默认 1 笔，未达标即 `force_sample_collection`，盘前 dry-run 和盘中动态资金计划都会读取该决策，形成“假设 -> 受控样本 -> 复盘 -> 晋级/收紧/观察”的自动闭环。
