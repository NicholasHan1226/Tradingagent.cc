...
- **前端终端运营层候选（2026-07-11）**：在既有 Hyperliquid 连续终端语言上增加六市场状态带和五域证据健康；统一 `pending / completed / review` 状态解析；过程页新增按时间/序列排序的来源/延迟事件账本；持仓快照和模拟账本可选透传数量、均价、现价、成本、市值、当日盈亏与来源；风险账本纳入滞后/异常/隔离证据域；复盘直接展示置信度、影响、证据和自动校准。四类账本统一本地搜索、排序、列显隐，URL 保存 `page/market/range`，支持 `Alt+1…6`、`Alt+←/→` 和 `/` 快捷键。仍是只读展示，不改变队列、资金、策略、cron、执行器、账户或实盘边界；移动端继续延期。
