...
- **复盘与自我演化验收**：2026-07-09 修复 A股 server-local simulated 成交进入日报复盘时的字段传递口径，复盘读取 local_sim 时必须保留 `filled_price/avg_price`、`trade_timestamp_bj`、`ashare_session_valid`、`fill_price_source`、`fill_price_source_class`、`fill_evidence`、`candidate_pool_layer`、`execution_source` 等样本判断字段；`fill_price_source=signal_card.price` 且有真实成交价的 A股 server-local 成交可作为明确成交价来源，缺成交价仍只作链路验证样本。新增 `shared.runtime_test.self_evolution_health`，只读检查各市场账本策略样本是否被演化层看到、风格权重 action 与最终 overlay 是否一致、是否有正向演化证据；该检查已接入 `full_acceptance --profile prod`，发现复盘/演化断链时给 warn，不直接阻断交易闭环。
