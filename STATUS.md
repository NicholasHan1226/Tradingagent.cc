...
- **市场覆盖与显式归因终端已发布（2026-07-11）**：设计/计划提交 `4c43715`、功能提交 `078e5d1` 已推送 GitHub `main` 并构建到生产。只读快照新增 `marketPulseCoverage`：六市场逐项标明 `sourced / no_representative / unavailable / degraded`，缓存仍保留原始获取时间；当前真实公开快照为 A 股 `1/1` 已取到，US/Crypto/HK/PM/CNFutures 均明确为无代表映射，前端不会猜测 BTC、期货合约或预测市场 ID。机会关联条只按相同显式 `opportunityId` 展示关联信号、持仓与可归因 PnL；持仓缺关联时保持 `—`，不做同标的推断。发布前 lint、40 个测试文件/187 项测试、客户端/API 构建通过；本地 1440×900 与 1280×720 均无横向溢出或控制台错误。生产服务 `tradingagent-front-api.service=active`，内部 health 与公开页面/快照均为 HTTP 200，公开资源为 `index-BrtqjPAv.js` / `index-B9iaovzO.css`；生产源码为 `078e5d1`，回滚目录戳 `20260711141550`。仍不改变队列、资金、策略、cron、执行器、账户或实盘边界。

- **市场因果终端已发布（2026-07-11）**：设计提交 `2a831cd`、功能提交 `0ebf81f` 已同步 GitHub 与生产源码。前端新增只读 SharedSignals 行情脉冲、真实点位 sparkline、按 `opportunity` URL 关联的机会周期/原始事件筛选、`⌘K` 命令面板、焦点闭环与版本化本地密度/列偏好；缺行情继续显示 `—`，不合成价格。生产使用 Node v24.4.1 构建，资源为 `index-BO0Kz8nR.js`，`tradingagent-front-api.service` active，内部健康与公开页面/快照均为 HTTP 200，公开构建标识为 `20260711-market-causal-terminal`，公开快照已返回 `marketPulses` 和 `sharedSignalsMarketPulse` 来源。发布前 lint、40 个测试文件/182 项测试、客户端/API 构建通过，quick acceptance 本地环境 195 项通过；1440×900 与 1280×720 无横向溢出，命令面板、焦点闭环与密度切换通过真实浏览器验证。生产系统 Python 未安装 pytest，因此生产 quick acceptance 只完成了入口检查并因依赖缺失退出，不作为代码失败；生产构建、服务、内部/公开快照已分别验收。保留回滚戳 `20260711130915` 与 `20260711135014`，更早三组冗余前端/API 回滚副本已删除。仍不改变队列、资金、策略、cron、执行器、账户或实盘边界。
