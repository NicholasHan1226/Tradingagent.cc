...
- [x] 新增 `shared/runtime_test/ashare_preopen_dry_run.py`：在开盘前只读预演 A股日线覆盖、候选池评分、200,000 元模拟账户动态资金计划和执行门禁；只写 `shared/runtime_test/ashare_preopen_dry_run_latest.json` / history，不写 `signals/`、server-local ledger、pending、review 或实盘队列。
