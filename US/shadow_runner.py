#!/usr/bin/env python3
"""US Phase D P0 shadow runner writing only to signals/shadow."""
...
class USShadowRunner(BaseShadowRunner):
    """Generate simple US shadow signals and isolate them under shadow state dirs."""
    ...
    def __init__(
        ...
        data = market_data or USMarketData(config=cfg)
        sim = simulator or USSimulator(config=cfg, market_data=data)
        super().__init__("us", cfg, data, sim)
            ...
            signals.append(
                {
                    ...
                    "trade_date": date,
                    "price": price,
                    "strategy_name": "us_phase_d_p0_shadow",
                }
            )
    ...
    def write_shadow_record(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        card.update(
            {
                "order_id": card.get("order_id") or f"US-SHADOW-{date}-{symbol}-{uuid.uuid4().hex[:8]}",
                "market": "us",
