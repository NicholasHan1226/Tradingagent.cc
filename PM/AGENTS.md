...
- 源目录: `tradingagent/PM/`。
- 数据入口: SharedSignals API 只提供 PM 市场/价格；CLOB sandbox 仅作模拟/研究适配，不替代 SharedSignals 供数层。
