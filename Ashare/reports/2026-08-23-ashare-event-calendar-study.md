...
- **数据与口径**：Tushare `margin` 交易所汇总接口区间分页拉取 2010-04 → 2026-08 全历史
  ...
  完全同源（10 日收益 × REGIME_BINS）。配对样本受共享指数缓存起点限制为
  **2018-01 → 2026-08 共 2086 天**（脚本输出已标注该窗口）。
- **首读结果（research_only，非晋级证据）**：

| 市场阶段 | n | 融资余额20日变化均值 | 中位 |
|---|---|---|---|
| weak | 627 | −2.4% | −1.9% |
| sideways | 801 | +0.8% | +0.5% |
| strong | 638 | +4.7% | +3.3% |

| 融资动量五分位 | 后10日指数均值 | 胜率 |
|---|---|---|
| Q1（最缩） | +0.6% | .600 |
| Q2–Q4 | −0.2%~+0.3% | .480–.547 |
| Q5（最胀） | −0.2% | .493 |

- **结论**：(a) **杠杆收缩与弱市强共变且单调**——弱市对应显著去杠杆、强市对应加杠杆，
  这为「弱市解禁修复是唯一成本后存活分层」提供了机制侧证据（弱市≈强制去杠杆后的
  抛压出清期）。(b) **融资动量作为指数择时因子判 FAIL**：五分位 spread −0.8%、
  Q1 反而最优（轻微反转味道）、corr(chg20, fwd10)=−0.045 无可用预测力，不予立项。
- **边界与未验证项**：2015 杠杆牛熊未进配对窗（指数缓存 2018 起），全样本复算需先延长
  缓存起点；个股级明细（单日约 2000 行 < 上限，可安全逐日拉）的「解禁前杠杆异动」
  窗口研究尚未开始；全部为历史条件分布，不得写入任何晋级材料。
