...
- 当前公共仓库的 pull-request CI 是日常代码合并 gate：正常 PR 只有在当前 head SHA 的配置测试全部通过后才可自动合并；与 `main` 新提交存在精确文件重叠时必须更新并重跑，shared core/deployment/dependency authority 在 `main` 任意前进后必须基于 fresh `main` 重新集成，governance-only 变更仅在同期 `main` 也修改 governance/core authority 时要求刷新；与主线变更文件和 authority domain 均不重叠的 market-local 变更可保留已测试 head 并行合并。`main` 合并后的 exact-SHA CI/打包证据是 GitHub 自动生产部署的代码 gate。Actions/CI 只管理 Git 集成与部署证据，不是 runtime/data truth，也不授予策略晋级、风险扩张、broker 或真实交易权限。若 Actions 临时不可用，已运行的市场 core、数据采集和模拟生命周期继续按各自 runtime evidence 运转，但普通代码 PR 不得因此绕过 `main` gate；独立 fallback runner 需另行形成同等级机器证据后才能替代。
