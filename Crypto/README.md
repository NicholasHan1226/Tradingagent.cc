...
failure，也不统计 `data_gap`、`risk_reject` 或普通 `decision` 事件。CLI 异常失败时只向
stderr 输出固定的 `round_trip_health_*` 原因码；对 checksum-verified 但 stale 的健康
快照，CLI 会先输出原有只读 JSON 再以非零退出告警。两种输出均不会包含原始异常、路径
或凭据，且只用于定位，不改变核心 accumulator、资本或晋级权限。
...
G5 acceptance 保持每日 09:05 的只读 gate，但 `Persistent=false`：发布或 timer
重启后错过的日程不补跑，避免迟到的全量审计与 5 分钟 core writer 互斥。它不会补造
任何证据；下一次准点日程仍重新验证相同的 48 小时/90% 门禁，也不改变资本、订单、
学习启用或真实交易权限。
...
> `forty_symbol_observation_runtime.py`。其 CLI 失败时仅输出固定的 `runtime_*`
> 原因码，不输出原始异常、路径或凭据；原因码只用于只读故障定位，不授予资本、订单或
> 晋级权限。40 币因子投影使用 feature set
...
gap/reject/ineligible 失败关闭且不跨缺口拼接。它以
