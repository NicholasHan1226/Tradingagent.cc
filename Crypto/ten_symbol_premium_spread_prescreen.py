"""Offline B-class premium+spread family factor pre-screen over ten-symbol 5m history.

This module evaluates the two frozen B-class candidate families
(``premium_momentum`` and ``spread_regime``) from the stage-2 hypothesis
generator against historical 5-minute bars plus the perp premium-index 5m
series and a per-symbol realized half-spread regime value (the latest
sufficient UTC day bucket p75, supplied by the caller as a research artifact).
...
and never runs by default inside the repository.  The ``--fetch`` path only
ever talks ``GET /v1/catalog`` + ``POST /v1/query`` and never reads SQLite or
any TradingDatas database.
"""
...
PREMIUM_SPREAD_PRESCREEN_CONTRACT = (
    "tradingagent.crypto.ten_symbol_premium_spread_prescreen.v1"
)
PREMIUM_SPREAD_RAW_CONTRACT = (
    "tradingagent.crypto.ten_symbol_premium_spread_prescreen_raw.v1"
)
...
FAMILY_IDS = ("spread_regime", "premium_momentum")
PREMIUM_DATASET_SUFFIX = ".premium_index"
PREMIUM_FIELDS = ("symbol", "open_time", "close")
PREMIUM_IDENTITY_FIELDS = ("symbol", "open_time")

_FEE = TAKER_FEE_RATE
_SLIP = SLIPPAGE_BPS / Decimal("10000")


class CryptoTenSymbolPremiumSpreadPrescreenError(RuntimeError):
    """Stable fail-closed error for premium+spread pre-screen fetch or analysis."""
...
def _canonical_json(value: Any) -> str:
    try:
        ...
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_payload_not_canonical"
        ) from exc
...
def _assert_simulation_only() -> None:
    if os.environ.get("REAL_TRADING_ENABLED") != "false":
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_real_trading_must_be_disabled"
        )
...
def _parse_utc(value: Any) -> datetime:
    if not isinstance(value, str) or not value:
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_timestamp_invalid"
        )
    try:
...
def _aligned_slot(value: datetime) -> datetime:
    if (
        ...
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_slot_invalid"
        )
    return value
...
def _decimal(value: Any, *, positive: bool = False) -> Decimal:
    if not isinstance(value, str):
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_decimal_invalid"
        )
    try:
...
def _premium_dataset_id(symbol: str) -> str:
    return f"crypto.perp.binance.{symbol.lower()}{PREMIUM_DATASET_SUFFIX}"


# ---------------------------------------------------------------------------
# Frozen premium+spread candidate grid (derived from the stage-2 generation
# config)
# ---------------------------------------------------------------------------


def _premium_spread_candidates() -> list[dict[str, Any]]:
    families = [
        family
    ...
    if [family["family_id"] for family in families] != list(FAMILY_IDS):
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_family_drift"
        )
...
def _validate_bar_rows(
    ...
    for row in rows:
        if not isinstance(row, Mapping) or set(row) != set(BAR_FIELDS):
            raise CryptoTenSymbolPremiumSpreadPrescreenError(
                "premium_spread_prescreen_row_shape_invalid"
            )
        ...
        if close_time != open_time + FIVE_MINUTES - timedelta(milliseconds=1):
            raise CryptoTenSymbolPremiumSpreadPrescreenError(
                "premium_spread_prescreen_bar_continuity_invalid"
            )
        ...
        if not isinstance(trade_count, int) or trade_count < 0:
            raise CryptoTenSymbolPremiumSpreadPrescreenError(
                "premium_spread_prescreen_trade_count_invalid"
            )
        if low > min(open_price, close) or high < max(open_price, close) or low > high:
            raise CryptoTenSymbolPremiumSpreadPrescreenError(
                "premium_spread_prescreen_ohlc_invalid"
            )
        if previous_open is not None:
            if open_time <= previous_open:
                raise CryptoTenSymbolPremiumSpreadPrescreenError(
                    "premium_spread_prescreen_rows_not_increasing"
                )
...
def _validate_premium_rows(
    rows: Sequence[Mapping[str, Any]], *, symbol: str
) -> list[dict[str, Any]]:
    validated: list[dict[str, Any]] = []
    seen: set[datetime] = set()
    for row in rows:
        if not isinstance(row, Mapping) or set(row) != set(PREMIUM_FIELDS):
            raise CryptoTenSymbolPremiumSpreadPrescreenError(
                "premium_spread_prescreen_row_shape_invalid"
            )
        if row.get("symbol") != symbol:
        ...
        if open_time in seen:
            raise CryptoTenSymbolPremiumSpreadPrescreenError(
                "premium_spread_prescreen_duplicate_timestamp"
            )
        seen.add(open_time)
        level = _decimal(row.get("close"))
        validated.append({"open_time": open_time, "close": level})
    return sorted(validated, key=lambda item: item["open_time"])


# ---------------------------------------------------------------------------
# Fetch (explicitly injected loopback transport only; never SQLite)
# ---------------------------------------------------------------------------
    ...
    *,
    ...
    for _ in range(max_windows):
        ...
        if (
            ...
            raise CryptoTenSymbolPremiumSpreadPrescreenError(
                "premium_spread_prescreen_fetch_metadata_invalid"
            )
        ...
        for row in page:
            ...
            if open_time < lower or open_time > upper:
                raise CryptoTenSymbolPremiumSpreadPrescreenError(
                    "premium_spread_prescreen_fetch_window_overflow"
                )
        receipts.append(
            {
        ...
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_fetch_window_budget_exceeded"
        )
    if not rows:
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_fetch_empty_history"
        )
    return rows, receipts


def _fetch_symbol_premium(
    client: SharedSignalsV1Client,
    *,
    catalog: CatalogEnvelope,
    symbol: str,
    ...
    dataset_id = _premium_dataset_id(symbol)
    rows: list[dict[str, Any]] = []
    receipts: list[dict[str, Any]] = []
    lower = start_open_time
    for _ in range(max_windows):
        ...
        run = collect_query_pages(
            ...
            identity_fields=PREMIUM_IDENTITY_FIELDS,
            max_pages=1,
            max_rows=PAGE_LIMIT,
        )
        envelope = run.envelope
        ...
        if (
            ...
            raise CryptoTenSymbolPremiumSpreadPrescreenError(
                "premium_spread_prescreen_premium_fetch_metadata_invalid"
            )
        page = [dict(row) for row in envelope.data]
        rows.extend(page)
        if upper >= end_open_time:
    ...
    if not rows:
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_fetch_empty_premium_history"
        )
    return rows, receipts
...
def fetch_raw_history(
    *,
    ...
    """Pull bars + premium-index history and persist canonical per-symbol raw files."""

    _assert_simulation_only()
    if not isinstance(client, SharedSignalsV1Client):
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_client_invalid"
        )
    start = _aligned_slot(start_open_time)
    end = _aligned_slot(end_open_time)
    if end < start:
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_window_invalid"
        )
    if tuple(symbols) != OBSERVATION_SYMBOLS:
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_symbols_invalid"
        )
    directory = Path(raw_dir)
    if directory.exists() and (directory.is_symlink() or not directory.is_dir()):
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_raw_dir_invalid"
        )
    ...
    for symbol in symbols:
        ...
        premium_rows, premium_receipts = _fetch_symbol_premium(
            client,
            catalog=catalog,
            symbol=symbol,
            start_open_time=start,
        ...
        premium = _validate_premium_rows(premium_rows, symbol=symbol)
        payload = {
            "contract": PREMIUM_SPREAD_RAW_CONTRACT,
            "symbol": symbol,
            "bars_dataset_id": _bar_dataset_id(symbol),
            "premium_dataset_id": _premium_dataset_id(symbol),
            "premium_source": "tradingdatas_loopback_query",
            ...
            "premium_receipts": list(premium_receipts),
            "bars_sha256": _sha256([dict(row) for row in bar_rows]),
            "premium_sha256": _sha256([dict(row) for row in premium_rows]),
    ...
    return {
        "contract": PREMIUM_SPREAD_PRESCREEN_CONTRACT,
        "event_type": "premium_spread_fetch_summary",
...
def _write_file_atomic(path: Path, encoded: bytes) -> None:
    ...
    try:
        ...
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_raw_write_failed"
        ) from exc
    finally:
...
def _read_raw_file(path: Path, *, symbol: str) -> dict[str, Any]:
    try:
        ...
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_raw_file_invalid"
        ) from exc
    try:
    ...
    except CryptoTenSymbolPremiumSpreadPrescreenError:
        raise
    except OSError as exc:
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_raw_file_invalid"
    ...
    if (
        payload.get("contract") != PREMIUM_SPREAD_RAW_CONTRACT
        or payload.get("symbol") != symbol
        or payload.get("bars_dataset_id") != _bar_dataset_id(symbol)
        or payload.get("premium_dataset_id") != _premium_dataset_id(symbol)
    ):
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_raw_contract_invalid"
        )
    for key, expected in _non_evidence_fields().items():
        if payload.get(key) != expected:
            raise CryptoTenSymbolPremiumSpreadPrescreenError(
                "premium_spread_prescreen_raw_authority_invalid"
            )
    bar_rows = payload.get("bars")
    premium_rows = payload.get("premium")
    if not isinstance(bar_rows, list) or not bar_rows:
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_raw_bars_invalid"
        )
    if not isinstance(premium_rows, list) or not premium_rows:
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_raw_premium_invalid"
        )
    bars, gaps = _validate_bar_rows(bar_rows, symbol=symbol)
    premium = _validate_premium_rows(premium_rows, symbol=symbol)
    return {"payload": payload, "bars": bars, "premium": premium, "gaps": gaps}
...
def load_raw_dir(
    ...
    *,
    ...
    for symbol in expected_symbols:
        ...
        premium_by_time = {item["open_time"]: item["close"] for item in loaded["premium"]}
        aligned[symbol] = {
            "bars": loaded["bars"],
            "premium": premium_by_time,
            "spread_p75_bps": None,
        }
        meta[symbol] = {
            "row_count": len(loaded["bars"]),
            "premium_count": len(loaded["premium"]),
            "first_open_time": _iso(loaded["bars"][0]["open_time"]),
            "last_open_time": _iso(loaded["bars"][-1]["open_time"]),
            "gap_count": len(loaded["gaps"]),
            "premium_source": loaded["payload"].get("premium_source"),
...
def _signal_snapshot(features: Mapping[str, Decimal]) -> dict[str, Any]:
    """Build a frozen time_series_momentum_v1 snapshot (mirrors factor_prescreen)."""
    ...
    for symbol, material in aligned.items():
        ...
        for index in range(12, len(ordered)):
            ...
            rows[slot] = {
                ...
                - Decimal("1"),
                "return_15m": by_open[slot]["close"]
                / by_open[window_opens[9]]["close"]
                - Decimal("1"),
                "by_open": by_open,
                "premium": material["premium"],
                "spread_p75_bps": material.get("spread_p75_bps"),
            }
        universe[symbol] = rows
    return universe


def _premium_at(premium: Mapping[datetime, Decimal], slot: datetime) -> Decimal | None:
    return premium.get(slot)
...
def _evaluate_premium_momentum(
    ...
    *,
    ...
    threshold = Decimal(parameters["premium_threshold"])
    ...
    for symbol in symbols:
        ...
        for slot in sorted(universe[symbol]):
            ...
            current_premium = _premium_at(premium, slot)
            past_premium = _premium_at(premium, past_slot)
            if current_premium is None or past_premium is None:
                continue
            premium_change = current_premium - past_premium
            universe_rows.append((symbol, slot, row["forward_return"]))
            if premium_change >= threshold:
...
def _evaluate_spread_regime(
    ...
    *,
    ...
    threshold = Decimal(str(parameters["regime_threshold_bps"]))
    mode = parameters["mode"]
    if mode not in ("narrow_only", "wide_only"):
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_spread_mode_invalid"
        )
    # rows: (symbol, slot, p75_bps, forward_return, forward_gross, signaled, gated)
    rows: list[tuple[str, datetime, Decimal, Decimal, Decimal, bool, bool]] = []
    per_symbol: dict[str, dict[str, Any]] = {}
    regime: dict[str, dict[str, Any]] = {}
    for symbol in symbols:
        symbol_rows = universe[symbol]
        if not symbol_rows:
            per_symbol[symbol] = {
                "signal_count": 0,
                "hit_rate": None,
            ...
            regime[symbol] = {"spread_p75_bps": None, "gated": False}
            continue
        p75 = next(iter(symbol_rows.values()))["spread_p75_bps"]
        p75_decimal = (
            Decimal(str(p75)) if p75 is not None else None
        )
        gated = (
            p75_decimal is not None
            and (
                p75_decimal <= threshold
                if mode == "narrow_only"
                else p75_decimal > threshold
            )
        )
        regime[symbol] = {
            "spread_p75_bps": _text(p75_decimal),
            "gated": gated,
        }
        entry = {"signal_count": 0, "returns": [], "positive": 0}
        for slot in sorted(symbol_rows):
            row = symbol_rows[slot]
            features = {"return_1h": row["return_1h"], "return_15m": row["return_15m"]}
    ...
    gated_rows = [row for row in rows if row[6]]
    selected = [(row[1], row[3], row[4]) for row in gated_rows if row[5]]
    universe_slots = sorted({row[1] for row in gated_rows})
    baseline = [(row[1], row[3]) for row in gated_rows]
    baseline_mean = (
    ...
    metrics = _sample_metrics(
        selected,
        universe_count=len(gated_rows),
        baseline_mean=baseline_mean,
        universe_slots=universe_slots,
        stride=horizon_bars,
        horizon_bars=horizon_bars,
    )
    kept_slots = set(sorted(universe_slots)[::horizon_bars])
...
def analyze(
    ...
    *,
    ...
    """Evaluate the two premium+spread families across every allowed horizon."""
    ...
    if (
        ...
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_horizon_invalid"
        )
    ...
    data_window = (
        ...
        if meta is not None
            ...
            symbol: {
                ...
                "premium_count": len(material["premium"]),
    ...
    spread_coverage = {
        symbol: {
            "spread_p75_bps": _text(aligned[symbol].get("spread_p75_bps")),
            "available": aligned[symbol].get("spread_p75_bps") is not None,
        }
        for symbol in symbols
    }
    candidates: list[dict[str, Any]] = []
    for candidate in _premium_spread_candidates():
        family = candidate["family"]
        parameters = candidate["parameters"]
        horizon_metrics: dict[str, Any] = {}
        for horizon in ALLOWED_HORIZON_BARS:
            universe = universes[horizon]
            if family == "premium_momentum":
                evaluated = _evaluate_premium_momentum(
                    universe, symbols=symbols, parameters=parameters, horizon_bars=horizon
                )
            elif family == "spread_regime":
                evaluated = _evaluate_spread_regime(
                    universe, symbols=symbols, parameters=parameters, horizon_bars=horizon
                )
            else:
                raise CryptoTenSymbolPremiumSpreadPrescreenError(
                    "premium_spread_prescreen_family_invalid"
                )
            metrics = dict(evaluated["metrics"])
            if (
                family == "premium_momentum"
                and "non_overlapping" in metrics
            ):
                kept_slots = set(sorted(evaluated["universe_slots"])[::horizon])
                subset_mean = (
                    Decimal(metrics["non_overlapping"]["mean_net"])
                    if metrics["non_overlapping"]["mean_net"] is not None
                    else None
            ...
            if family == "spread_regime":
                horizon_entry["regime"] = evaluated["regime"]
            horizon_metrics[f"h{horizon}"] = horizon_entry
        candidates.append(
            {
    ...
    return {
        "contract": PREMIUM_SPREAD_PRESCREEN_CONTRACT,
        "event_type": "premium_spread_factor_prescreen_analysis",
        "symbols": list(symbols),
        "data_window": data_window,
        "premium_source": meta.get("premium_source") if meta else None,
...
def render_report(result: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        " OHLCV bar + 同币 perp premium-index 5m 序列（`crypto.perp.binance."
        "<symbol>.premium_index`，字段 `close` 即 funding proxy level）。",
        "- premium 数据来源：见下方 `premium_source` 与数据窗口表；本报告的 premium"
        " 序列来自服务器只读诊断抽取（生产 18083 对 `.premium_index` 的 query 因"
        " `transport_profile_unverified` fail-closed），`authority=none`、"
        "`research_only=true`。",
        "- spread 数据来源：`spread_regime` 族使用实测点差投影 artifact 的 per-symbol"
        " 最新充足 UTC 日 p75 半点差（bps）作为静态 regime 值；本报告 spread 只覆盖"
        " 单日（见下方数据治理说明），缺失 symbol 按无充分样本处理。",
    ...
    for symbol in result["symbols"]:
        item = window[symbol]
        lines.append(
            f"| {symbol} | {item['row_count']} | {item['premium_count']}"
            f" | {item['first_open_time']} | {item['last_open_time']}"
            f" | {item.get('gap_count', 0)} | {item.get('premium_source', '—')} |"
        )
    header = (
    ...
    lines += [
        ...
        "---",
        "",
        f"生成：`Crypto/ten_symbol_premium_spread_prescreen.py --report`；contract "
...
def _build_fetch_client(
    *,
    ...
    try:
        ...
        raise CryptoTenSymbolPremiumSpreadPrescreenError(
            "premium_spread_prescreen_transport_invalid"
        ) from exc
    return SharedSignalsV1Client(
        SharedSignalsV1Config(
            ...
            | frozenset(_premium_dataset_id(symbol) for symbol in OBSERVATION_SYMBOLS),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Offline ten-symbol premium+spread factor pre-screen research"
    )
    ...
    try:
        ...
        if args.fetch:
            ...
            if missing:
                raise CryptoTenSymbolPremiumSpreadPrescreenError(
                    "premium_spread_prescreen_fetch_arguments_incomplete"
                )
        ...
        print(
            "crypto ten-symbol premium spread prescreen failed closed",
            file=sys.stderr,
        )
        return 2
...
__all__ = [
    ...
    "PREMIUM_SPREAD_PRESCREEN_CONTRACT",
    "PREMIUM_SPREAD_RAW_CONTRACT",
    "CryptoTenSymbolPremiumSpreadPrescreenError",
    "analyze",
    "fetch_raw_history",
    "load_raw_dir",
    "main",
